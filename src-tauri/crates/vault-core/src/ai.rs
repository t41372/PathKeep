...
pub async fn answer_history_question(
    ...
    connection.execute(
        "INSERT INTO ai_assistant_runs
         (question, answer, provider_id, embedding_provider_id, citations_json, notes_json, created_at)
        ...
        params![
            ...
            serde_json::to_string(&final_citations)?,
            serde_json::to_string(&search_response.notes)?,
            now_rfc3339()
        ],
    )?;
...
mod tests {
    ...
    fn ai_index_status_warns_when_archive_is_missing() {
        let paths = test_paths();
        let status = ai_index_status(&paths, &base_config(), None).expect("status");
