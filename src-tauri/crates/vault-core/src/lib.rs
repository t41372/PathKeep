...
pub use ai::{
    ...
    DEFAULT_STATIC_MODEL_FILES, DEFAULT_STATIC_MODEL_REPO, EMBEDDING_FINGERPRINT_VERSION,
    EmbeddingDescriptor, EmbeddingDtype, EmbeddingFingerprint, EmbeddingPooling, EmbeddingProvider,
    EmbeddingRole, ExternalEmbeddingProvider, IndexBackfillLedger, IndexBackfillProgress,
    LlmCapabilities, LlmChatRequest, LlmChatResponse, LlmChunkStream, LlmMessage, LlmProvider,
    LlmResponseFormat, LlmRole, LlmStreamChunk, LlmToolDef, LlmUsage, MAX_WORKING_SET,
    ModelDownloadProgress, ModelFile, NarrativeSummary, NoopDownloadProgress, QWEN3_QUERY_TASK,
    QueryFamilyFacts, RigLlmProvider, STATIC_INAPP_BASE_URL, STATIC_MAX_INPUT_TOKENS,
    StaticEmbeddingMatrix, StaticEmbeddingProvider, TopicFacts, VectorIndex, VectorStore,
    VectorStoreHeader, VisitContentMap, WorkingSetCandidate, WorkingSetConfig, ai_index_status,
    ai_queue_status, answer_history_question, answer_history_question_with_control,
    apply_role_instruction, build_ai_index, build_ai_index_with_control, build_dedup_content_hash,
    candle_repo_for_runtime, content_key_from_hash, degrade_candle_to_external,
    degrade_static_to_external, deregister_ai_chat_run, drive_ai_chat_stream,
    ensure_model_downloaded, gguf_file_name, load_assistant_run_response, model_dir_for_repo,
    model_is_loadable, model_is_present_and_verified, parse_static_config, preview_ai_integrations,
