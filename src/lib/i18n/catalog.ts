...
const zhCnM3Namespaces = {
  intelligence: {
    ...
    statusFailedDescription:
      '最近一次语义索引任务失败。修复提供方或模型问题后，请检查队列并重试。',
    statusDegradedLabel: '语义检索已退化',
    statusDegradedDescription:
      '搜索仍可回退到关键词召回，但语义提供方当前不可完全使用。',
  ...
  explorer: {
    ...
    semanticPrompt:
      '只有在输入自然语言查询后，语义和混合召回才会运行。下方的 canonical 归档列表仍会继续应用筛选条件。',
    ...
    noSemanticDescription:
      '语义召回没有为此查询找到足够强的证据。混合模式仍会在下方保留 canonical 关键词结果。',
    loadingArchive: '正在搜索 canonical 归档',
  ...
  assistant: {
    ...
    noLlmProviderSelected: '尚未选择 LLM 提供方',
    llm: 'LLM',
    retrieval: '检索',
    unset: '未设置',
    ...
    providerReachable: '提供方可达',
    providerNeedsAttention: '提供方需要关注',
...
const zhTwM3Namespaces = {
  intelligence: {
    ...
    statusFailedDescription:
      '最近一次語義索引工作失敗。修復提供者或模型問題後，請檢查佇列並重試。',
    statusDegradedLabel: '語義檢索已退化',
    statusDegradedDescription:
      '搜尋仍可回退到關鍵字召回，但語義提供者目前無法完整使用。',
    statusBlockedLabel: '封存設定未完成',
    statusBlockedDescription:
      '請先初始化並解鎖封存，再使用 PathKeep 的智慧功能。',
    statusDisabledLabel: 'AI 已停用',
    statusDisabledDescription:
      '核心封存功能仍可使用，但語義搜尋和助手目前已關閉。',
  ...
  explorer: {
    ...
    exportDescription:
      '匯出只會使用目前可見的封存範圍。M1 先交付造訪記錄匯出；更豐富的內容型證據會在後續版本補上。',
    copied: '已複製',
    noResultEyebrow: '詳情',
  ...
  assistant: {
    ...
    archiveNotInitializedDescription:
      'AI 助手需要已初始化的封存，才能搜尋你的瀏覽歷史。',
    lockedTitle: '助手需要已解鎖的封存',
    lockedDetail: '請先解鎖封存，再載入帶證據的助手回答。',
    lockedEyebrow: '已鎖定',
    disabledTitle: '助手目前已停用',
    disabledBody:
      'PathKeep 在沒有 AI 的情況下仍可使用封存。需要帶證據的回答時，請到設定中啟用 AI 分析和助手。',
    statusEyebrow: '助手狀態',
    testProvider: '測試提供者',
    refreshQueue: '重新整理佇列',
    openSettings: '開啟設定',
    ...
    runningContext: '執行內容',
    noLlmProviderSelected: '尚未選擇 LLM 提供者',
    llm: 'LLM',
    retrieval: '檢索',
    unset: '未設定',
    ...
    runningLabel: '執行中',
    queuedJobLabel: '助手 · #{id}',
    providerReachable: '提供者可達',
    providerNeedsAttention: '提供者需要關注',
    ...
    emptyEyebrow: '證據優先',
    emptyTitle: '開始一條歷史研究線索',
    emptyDescription:
      '助手只會根據封存證據作答。提出一個有範圍的問題，PathKeep 會引用它所依賴的造訪記錄。',
  ...
  insights: {
    ...
    latestRunGrowthBody:
      '最近一次成功執行新增了 {visits} 次造訪、{urls} 個 URL 和 {downloads} 筆下載。',
...
> = {
  ...
  'zh-CN': {
    ...
    settings: {
      ...
      derivedOnly: '仅衍生层',
      derivedStateBoundaryTitle: 'Canonical 与 derived 的边界',
      derivedStateBoundaryBody:
        '这些控件只会触及 enrichment 和 insight 表，不会改动 canonical archive visit、manifest 或 rollback 状态。',
      ...
      clearCompletedBody:
        '清除了 {enrichments} 条富化记录、{features} 条特征记录和 {cards} 张 insight 卡片，canonical history 未受影响。',
  ...
  'zh-TW': {
    ...
    settings: {
      ...
      derivedOnly: '僅衍生層',
      derivedStateBoundaryTitle: 'Canonical 與 derived 的邊界',
      derivedStateBoundaryBody:
        '這些控制只會觸及 enrichment 與 insight 資料表，不會改動 canonical archive visit、manifest 或 rollback 狀態。',
      ...
      openAuditRun: '開啟審計執行',
      latestGrowthSignalBody:
        '執行 #{runId} 為封存範圍新增了 {visits} 次造訪、{urls} 個 URL 和 {downloads} 筆下載。',
      rebuildCompletedTitle: '衍生狀態已重建',
      rebuildCompletedBody:
        '處理了 {visits} 次造訪、刷新了 {enriched} 筆富化列，並產生了 {cards} 張 insight 卡片。',
      clearCompletedTitle: '衍生狀態已清除',
      clearCompletedBody:
        '清除了 {enrichments} 筆富化列、{features} 筆特徵列和 {cards} 張 insight 卡片，canonical history 未受影響。',
