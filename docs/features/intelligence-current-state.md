...
| Semantic search                           | Explorer                                                                                                   | **v0.1 disabled / v0.2 roadmap**                       | semantic / hybrid 仍保留同一路由 deep-link grammar，但 controls disabled，畫面只顯示 deferred 說明；v0.1 只開放 keyword / regex recall                                                                                                                                                                                                                                                                                                  |
| AI Assistant                              | Assistant                                                                                                  | **v0.1 disabled / v0.2 roadmap**                       | Assistant route 保留入口與 shared scope honesty，但不顯示問答、seed prompts、provider 測試或 queue 操作                                                                                                                                                                                                                                                                                                                                 |
| AI provider test                          | Settings                                                                                                   | **v0.1 disabled / v0.2 roadmap**                       | provider/API-key placeholder 必須 disabled 並標註後續版本；v0.1 不提供可操作的 provider probe                                                                                                                                                                                                                                                                                                                                           |
| AI queue review                           | Jobs、sidebar                                                                                              | **v0.1 hidden for optional AI jobs**                   | optional assistant / embedding queue 不在 v0.1 顯示可操作 queue；Jobs / sidebar 只呈現 deterministic runtime 與 archive workflow queue truth                                                                                                                                                                                                                                                                                            |
...
- `semantic` / `hybrid` UI 先 disabled，標記 `Coming in v0.2`。
- default build 不連結 LanceDB / vector runtime。
- 查詢路徑只允許 keyword / regex / deterministic evidence recall。
...
2. `readable-content-refetch`
   ...
   - v0.1.0 disabled；目前不補抓可讀正文，也不提供 site-adapter evidence
