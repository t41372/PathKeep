...
>
> **2026-04-19 calendar heatmap note:** 使用者已明確推翻上一輪「週內 × 小時主圖」的 accepted 假設；current truth 現在改成：`Browsing Rhythm` 主圖是 **GitHub 式真實日期日曆熱力圖**，資料直接來自 `getDiscoveryTrend(..., 'day')`，每個方格都對應一天真實日期。點某一天後，卡片下半部再顯示同一天的 digest / top sites / 24 小時分布，其中小時分布沿用 `getBrowsingRhythm(singleDayRange)`，但不再冒充主圖本身。這次同時也收口了版面與可維護性：`Search Activity` 和 `Activity Mix` 回到同一行 half-width；除了執行摘要 / 時段概覽 / 瀏覽節奏之外，其餘 intelligence 卡片一律不占 full-width；所有卡片都改成 capped body + internal scroll，不再讓單張低價值卡片把整頁高度撐爆。
>
