...
- explainability 主頁
- `/intelligence`、`/intelligence/day/:date` 與 `/intelligence/domain/:domain` 現在已收斂成同一套 shared entity grammar：day route 只保留 `profileId` query；domain route 沿用 `range` / `start` / `end` / `profileId`
