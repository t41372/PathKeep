...
>
> **2026-04-19 accepted override:** M7 當時暫留的 compare-set promotion / focus-query 缺口，現已由 [intelligence-aggregate-entity-focus-tradeoff.md](intelligence-aggregate-entity-focus-tradeoff.md) additive 收口。shipping truth 以 M6 + M7 baseline 再加上 M8 override 為準：`compare set` 已升格成 first-class shared route，而 non-overview shared entity routes 也已可接受受限的 `focusType` / `focusId`。
...
3. Integrations external outputs、Explorer grouped view、`/intelligence` overview 各自手寫 action hierarchy
...
- M7 當時先不為 shared routes 增加 `focus` / context-highlight param；若後續要補，必須以 additive shared contract 落地，不能回退成 consumer-local state
...
- `compare set` 在 M7 closeout 時先走 `trail insights`；後續是否升格由 M8 決定
...
1. `/intelligence`、day/domain pages、Explorer grouped/detail surfaces、Integrations external outputs 不再各自手寫 entity action strip
...
5. Integrations external outputs 的 top domain / discovery-date chips 改吃 shared entity href，而不是留 static label
6. M7 closeout 時 `domain insights` route 先不增加 context/focus query；context highlighting 留到 M8，且必須走 shared route contract 而不是 consumer-local state
...
這些後續已由 [M8 README](../plan/m8-aggregate-entity-identity/README.md) 與 [M8 trade-off](intelligence-aggregate-entity-focus-tradeoff.md) 收口，不再回頭用 `TODO: M7` 漂浮追蹤。
