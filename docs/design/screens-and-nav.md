...
| **Import**             | Takeout 導入 wizard + grouped scan report + recent batch review + `?batch=` deep-link + revert / restore；Google Takeout shipping scope 目前採 Chrome-first，必須明講哪些會導入、哪些只忽略、哪些需要人工複核                                                                                               |
