...
| **Import**             | Takeout 導入 wizard + Browser Direct local DB wizard + grouped scan report + current batch review + collapsed history / maintenance surface + `?batch=` deep-link + revert / restore；Google Takeout shipping scope 目前採 dedicated Chrome history payload，第一屏要用清楚的 `支援什麼 / 不會匯入什麼 / 怎麼拿到正確匯出` 結構直接告訴使用者「可以選 zip、資料夾或單一 JSON 檔，但內容必須是 Chrome history payload」；Browser Direct 則只把 Google Chrome / Safari 顯示成 validated import path，並走 `inspect_browser_history` / `import_browser_history`，不能把 Safari `History.db` 送進 Takeout parser。Safari 缺 Full Disk Access 時要留在 detected list 並顯示 needs-access guidance。單一大型 JSON / SQLite 匯入時，progress overlay 必須維持可見的 active state 與 indeterminate progress，而不是長時間停在假 `0 / 1 · 0%`。 |
