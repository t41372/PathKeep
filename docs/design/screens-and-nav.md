...
| **Import**             | Takeout 導入 wizard + grouped scan report + recent batch review + `?batch=` deep-link + revert / restore；Google Takeout shipping scope 目前採 Chrome-first，第一屏就要告訴使用者「目前只吃 dedicated Chrome history payload」，並教使用者在掃描前先檢查 zip / 資料夾裡是否真的有 `Chrome/BrowserHistory.json` / `Chrome/History.json` / 在地化等價檔；若偵測到 `My Activity` JSON/HTML，頁面必須直接說這不是目前支援的匯出類型，而不是只丟 generic ignored / no matches。 |
