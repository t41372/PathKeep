...
| **Intelligence**       | Core Intelligence 主頁：analysis snapshot、Insight Access、spotlight、research signals、evidence / health、runtime digest，以及 day / domain / query family / refind / session / trail 的 shared insights routes                                                                                            |
...
- `query family`、`refind page`、`session`、`trail` 現在也有正式 shared insights route：`/intelligence/query-family/:familyId`、`/intelligence/refind/:canonicalUrl`、`/intelligence/session/:sessionId`、`/intelligence/trail/:trailId`。除了 `day` 之外，這些 route 一律沿用 `range`、`start`、`end`、`profileId` query grammar。
- `/intelligence` 的 `Search Activity` 現在固定有 `engines / concepts / recent queries / families` 四個 tab。`Recent Queries` 只是 additive review surface：row 若已有 `familyId`，primary CTA 必須走 shared `query-family insights` route；其餘 drilldown 只允許 `trail insights` 與 Explorer evidence，不能藉機重開 Explorer `queries` view 或另一套 URL grammar。
- `refind` route 的 path identity 直接使用 encoded canonical URL；M7 不新增 synthetic ID layer。若日後要補 context focus / highlight，也不得先偷長 consumer-local query grammar。
...
- `reopened investigation`、`habit`、`stable source`、`friction`、`multi-browser diff` 這類 active entity 不再允許各自決定 destination：domain-based surface 一律走 `domain insights`，compare-set 一律走 `trail insights`，path-flow 只有在 step 可穩定解析為 registrable domain 時才提供 shared CTA。
