...
- Dashboard 也正式共用同一套 `Browsing Rhythm` 真實日期日曆熱力圖，但固定以**單一 calendar year** 呈現。若 archive 內有多個年份，卡片必須顯示當前查看年份，並以 bounded pager 在 `getDiscoveryTrend(..., 'day').availableYears` 之間前後翻頁；若 current year 存在於 `availableYears`，還要提供明確的 `回到當前年份` 捷徑。不得翻到不存在的未來年份。這張卡不受 `/intelligence` route time scope 影響。
