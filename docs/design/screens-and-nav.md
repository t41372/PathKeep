...
| **Import**             | Takeout 導入 wizard + grouped scan report + current batch review + collapsed history / maintenance surface + `?batch=` deep-link + revert / restore；Google Takeout shipping scope 目前採 dedicated Chrome history payload，第一屏就要直接告訴使用者「可以選 zip、資料夾或單一 JSON 檔，但內容必須是 Chrome history payload」，並教使用者在掃描前先檢查匯出裡是否真的有 dedicated history JSON；若偵測到 `My Activity` JSON/HTML，頁面必須直接說這不是目前支援的匯出類型，而不是只丟 generic ignored / no matches。 |
...
- Dashboard 也正式共用同一套 `Browsing Rhythm` 真實日期日曆熱力圖，但固定以**單一 calendar year** 呈現。若 archive 內有多個年份，卡片必須顯示當前查看年份，並以 bounded pager 在「最早有資料的年份」到 `max(當前年份, 最晚有資料的年份)` 的**連續年份帶**之間前後翻頁；中間空白年份仍要顯示空熱力圖，而當前年份永遠要存在並提供明確的 `回到當前年份` 捷徑。不得翻到這條連續年份帶之外的年份。這張卡不受 `/intelligence` route time scope 影響。
