...
| **Import**             | Takeout 導入 wizard + grouped scan report + current batch review + collapsed history / maintenance surface + `?batch=` deep-link + revert / restore；Google Takeout shipping scope 目前採 dedicated Chrome history payload，第一屏要用清楚的 `支援什麼 / 不會匯入什麼 / 怎麼拿到正確匯出` 結構直接告訴使用者「可以選 zip、資料夾或單一 JSON 檔，但內容必須是 Chrome history payload」；若偵測到 `My Activity` JSON/HTML，頁面必須直接說這不是目前支援的匯出類型，而不是只丟 generic ignored / no matches。單一大型 JSON 匯入時，progress overlay 必須維持可見的 active state 與 indeterminate progress，而不是長時間停在假 `0 / 1 · 0%`。 |
