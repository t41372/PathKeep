...
- `Browsing Rhythm` 主圖必須對應**真實日期**；目前 accepted contract 是 GitHub 式日曆熱力圖。`/intelligence` overview 與 Dashboard 的這張卡現在允許 **preview-first** 特例：點某一天先在卡內打開 compact inline preview，再由明確的 `查看詳情` CTA 進 `/intelligence/day/:date`。這個 preview 仍是次級資訊，不可膨脹成第二個完整 day route，也不可把這個 click grammar 套回其他 day entry surface。
- `Browsing Rhythm` 的 activity mix / proportion bar 圖例 swatch 必須使用不透明且可辨識的 semantic palette；不能引用不存在的 token，也不能讓某些 category 在 dark / light mode 變成透明或接近背景色。
...
- 凡是已經顯示 `domain` 或 `day` 實體的 active surface，不得再各自決定不同的 primary click behavior；shared entity 應優先帶去 `day/domain insights`，Explorer evidence 只作 secondary CTA。
- `證據與新鮮度` 這類 review badge 是 header chrome，不是內容列；它必須縮在 card 右上角並與 title 同列，hover / focus 命中區也只能覆蓋元件本身。
