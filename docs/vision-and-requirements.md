...
關於新世代的正式數據庫選型與分層邊界，見 `docs/database-selection-decision-2026-04-05.md`。
...
- **快照必須有保留上限**，不能無限累積 — 一個 100 GB 的 archive 保留 8 份快照就是 800 GB。
  - 預設保留最近 4-8 個 archive 快照，用戶可調。
  - 原生瀏覽器 DB 的季度 checkpoint 快照保留最近 8-16 個。
  - 超出保留上限的舊快照自動清理，但用戶可以在清理前手動匯出到外部存儲。
  - 設定頁面顯示快照佔用的磁碟空間。
...
- **Embedding 不能用 JSON 文本存儲**。向量應使用 binary blob 或專用向量格式，避免 2-3 倍的序列化膨脹。
- **語義搜尋不能做全表掃描**。幾千萬行 embedding 全量 cosine 計算不可能保持互動性，需要 ANN（近似最近鄰）索引。
- **AI 資產不能拖慢核心 archive**。如果 embedding 或向量索引增長到影響主庫的備份、VACUUM 或查詢效能，應將其分離到獨立的 sidecar 存儲。
- **設定頁面應顯示各類資產的磁碟佔用**：core archive、FTS 索引、embedding、enrichment、快照等，讓用戶清楚知道空間花在哪裡，以及最近的增長趨勢。
- 具體的向量存儲方案（SQLite Vec、LanceDB、Qdrant 等）留待 Intelligence 里程碑時根據實際數據量決定，不在此階段鎖定。
