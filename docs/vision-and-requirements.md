...
- PathKeep 不提供雲端上傳；跨機器遷移走使用者主動觸發的 Export / Import bundle（`.pathkeep-bundle` zip 寫到使用者選的本地路徑，由使用者自己決定要不要透過 USB / 自己的 cloud drive / sync 工具搬到另一台機器）。app 本身永遠不會主動把資料送到外部服務。
- 產品不收集使用行為資料；support / bug report 只走使用者手動提供的本地診斷資料。
- 開源。用戶可以審計所有代碼。
...
- [技術棧與平台](architecture/tech-stack.md) — 技術選型、數據庫分層、v0.2.0 deferred AI / vector boundary
