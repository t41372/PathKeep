...
**2026-04-10 boundary / release closeout (`WORK-M4-K` / `WORK-M4-L`)**：macOS 現在已在不改寫 `ADR-005` 的前提下正式接上 Touch ID session unlock；Settings 也新增 updater check / download / install / restart surface。另一方面，bundle / keyring / data-root namespace 已統一切成 `com.yi-ting.pathkeep`，release workflow 也補齊 updater `latest.json` / signatures / checksums contract、single-script version bump、release size attribution 與 code-health audit。剩餘未 shipping 的 plugin sandbox / queue-family / long-horizon intelligence 已明確移到 M5，而不是留在 M4。
