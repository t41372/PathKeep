...
| `src-tauri/crates/vault-core/src/insights.rs`    | 仍同時承載 legacy deterministic / insight paths 與 M5 後留下的舊分支，責任已比 M4 時更清楚，但檔案本體仍偏大。                                                                                                             | 等未來有專門的 post-M5 cleanup / extraction block 再處理，不再把它掛在已完成的 `WORK-M5-A` / `WORK-M5-B` 名下。                                    |
