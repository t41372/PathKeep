...
| Rust dependency graph for desktop builds         | Default desktop release still links optional intelligence runtime (`lancedb` / `lance` / `datafusion` / `rig-core`) into the same binary as archive / shell-critical flows.                                               | Follow-up bundle-size / runtime-boundary work is tracked separately; further big wins need explicit build-boundary decisions, not just local cleanup. |
