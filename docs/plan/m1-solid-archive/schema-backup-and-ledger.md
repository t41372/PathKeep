...
- 2026-04-09 closeout：M1 這裡現在多了一份明確的 truth matrix。shipping surface 是 canonical migration / backup / manifest / snapshot safety net / doctor baseline；import batch 的 un-revert 也已使用獨立 `restore` run type，而不是再冒充 `rollback`。當時尚未 shipping 的 snapshot restore preview / execute、legacy DB one-shot converter 與 richer approval metadata 已被誠實標成 deferred。
- 2026-04-10 closeout：`WORK-M1-D` 已把 recoverability contract 從 truth-only closeout 拉回 shipping：saved raw-source checkpoint 現在可經 `snapshot_restore` 做 Preview / Execute、local snapshot / export / staging / quarantine 也有 manual-first prune flow；rekey execute 會寫入 `rekey` run、manifest 與 safety snapshot artifact。仍未 shipping 的只剩「任何 archive-file snapshot 一律可自動 restore」這種過度承諾。
...
| Unified run / manifest / audit | backup / import / rollback / restore / doctor / snapshot_restore / rekey / retention_prune 已共用 `runs`，manifest / snapshot 透過 `AuditRunDetail` 暴露 user-facing artifact        | `load_audit_run_detail`、Dashboard / Audit read models、M2 import revert / restore tests、`command_helpers_cover_local_desktop_flows`                                            | approval reason / manual-step metadata 尚未成為 schema 欄位。                                                           |
| Snapshot safety net            | 每次 profile backup 會落 `raw-source-checkpoint` snapshot artifact；rekey execute 前一定建立 safety snapshot，且 backup checkpoint 現在可 preview / replay 成 `snapshot_restore` run | `canonical_backup_pipeline_writes_runs_manifests_snapshots_and_queries`、`rekey_archive_keeps_a_safety_snapshot`、`snapshot_restore_preview_and_run_record_the_saved_checkpoint` | archive-file safety snapshot 若需要舊 key，仍保持 manual recovery，而不是假裝任何 `.sqlite` snapshot 都能自動 restore。 |
...
| Recoverability contract        | rollback visibility、snapshot safety net、doctor repair、checkpoint replay restore、以及 manual-first retention prune 都已接到 `runs` story                                          | ADR-003 / ADR-004、M2 import revert / restore tests、`retention_preview_and_prune_clear_local_artifacts_and_record_a_run`                                                        | shipping restore 是 checkpoint replay，不是 destructive archive rollback；legacy DB one-shot converter 仍 deferred。    |
...
- [x] `M1-DB-SN-004` 設計 snapshot restore preview，至少能告知將回到哪個版本、覆蓋哪些資料、需要哪些手動確認。（2026-04-10，`WORK-M1-D`：Audit 現在可 preview saved raw-source checkpoint 的 replay scope / estimated rows；archive-file safety snapshot 若需要舊 key，保持 manual-first warning）
- [x] `M1-DB-SN-005` 為 restore 建立最小 command contract 和測試，即使完整 UI 在 M2 / M4 才深化。（2026-04-10，`WORK-M1-D`：`preview_snapshot_restore` / `run_snapshot_restore` + Rust / frontend coverage 已落地）
