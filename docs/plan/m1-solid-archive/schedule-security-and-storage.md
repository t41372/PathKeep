...
- 2026-04-29 scheduled-backup audit update：macOS `schedule_status` 會把已知 pre-`com.yi-ting.pathkeep` LaunchAgent label（`dev.codex.pathkeep.backup`、`dev.codex.browser-history-backup.backup`）回報為 `legacy-install-detected`，並列出 legacy plist / launchd service evidence。這維持 clean-break namespace 決策，不自動 migrate 或 remove legacy task；UI 必須引導使用者 review / manual cleanup，而不是把 legacy task 當成 canonical installed。
- 2026-04-29 scheduled-backup UI update：`/schedule` 保持原 route，但 sidebar label 改為 `Scheduled Backup Settings` 並移入 `SYSTEM`。主頁第一層現在直接提供 interval selection、current config/status、install / update、remove 與 legacy/error attention state；Onboarding schedule step 可選 interval 並選擇 setup 完成時安裝或 skip，skip 不做 scheduler mutation。
...
| Schedule PME + verify     | Preview / Manual / Execute / Verify 全部已有 UI 與 command surface；Scheduled Backup Settings 主頁可直接修改 interval、install / update、remove 並查看 status；Onboarding setup 可選擇 install 或 skip；macOS / Windows 支援 app apply / status / remove；macOS status 另會把 known legacy LaunchAgent 回報為 `legacy-install-detected`；Linux 維持 manual-review | `preview_schedule` / `apply_schedule` / `remove_schedule` / `schedule_status`、Schedule / Onboarding Vitest slices | Windows 實機 Task Scheduler acceptance 仍需在 release VM 上覆核；legacy macOS task 不會被自動 migrate/remove；Onboarding skip 不會 mutate native scheduler；Linux 不假裝自動 apply / detect 已完成。 |
...
- [x] `M1-OPS-SC-005` 建立 scheduler status detection，能回報 mismatch、legacy install、missing artifact、permission issue；macOS 走實際檔案比對，Windows / Linux 明確維持 manual-review。（2026-04-06 audit follow-up；2026-04-28 release blocker update：Windows 改走 Task Scheduler XML query / mismatch / permission-warning，Linux 仍 manual-review）
