...
| Schedule PME + verify     | Preview / Manual / Execute / Verify 全部已有 UI 與 command surface；macOS / Windows 支援 app apply / status / remove；macOS status 另會把 known legacy LaunchAgent 回報為 `legacy-install-detected`；Linux 維持 manual-review | `preview_schedule` / `apply_schedule` / `remove_schedule` / `schedule_status`                                 | Windows 實機 Task Scheduler acceptance 仍需在 release VM 上覆核；legacy macOS task 不會被自動 migrate/remove；Linux 不假裝自動 apply / detect 已完成。 |
