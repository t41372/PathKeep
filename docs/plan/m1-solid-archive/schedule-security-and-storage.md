...
| Schedule PME + verify     | Preview / Manual / Execute / Verify 全部已有 UI 與 command surface；macOS / Windows 支援 app apply / status / remove，Linux 維持 manual-review                                          | `preview_schedule` / `apply_schedule` / `remove_schedule` / `schedule_status`                                 | Windows 實機 Task Scheduler acceptance 仍需在 release VM 上覆核；Linux 不假裝自動 apply / detect 已完成。 |
