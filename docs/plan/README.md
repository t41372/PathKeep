...
>
> **2026-05-03 M14 closeout note**：`WORK-M14-A` 已完成並完成供應鏈 remediation。Explorer keyword recall 現在走 deterministic lexical recall v2：dependency-free lowercase / compact analyzer、SQLCipher-backed FTS5 unicode61 prefix + trigram projection、CJK 2/3-gram support fields、compact punctuation/space-insensitive recall、BM25 relevance default sort、以及 opaque relevance cursor。未經批准的 OpenCC / Unicode normalization 依賴已移除；繁簡中文與全形/半形 folding 只留作 future approved dependency / in-repo implementation follow-up。`WORK-M14-B` 只作 blocked follow-up，不把 fuzzy edit distance 塞進第一刀。
