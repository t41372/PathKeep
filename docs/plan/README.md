...
>
> **2026-05-03 M14 closeout note**：`WORK-M14-A` 已完成並完成供應鏈 remediation；`WORK-M14-C` 隨後恢復符合 trust gate 的 ICU4X NFKC / full-width normalization；`WORK-M14-D` 用官方 OpenCC 1.3.0 字典資產 + repo-owned Rust converter 恢復繁簡 folding。Explorer keyword recall 現在走 deterministic lexical recall v2：ICU4X NFKC + OpenCC-derived `t2s` / `tw2sp` variants + lowercase / compact analyzer、SQLCipher-backed FTS5 unicode61 prefix + trigram projection、CJK 2/3-gram support fields、compact punctuation/space-insensitive recall、BM25 relevance default sort、以及 opaque relevance cursor。未經批准的 OpenCC binding 依賴已移除；OpenCC native C++ linking 仍需先證明 CMake/C++/CI/package contract。`WORK-M14-B` 已解鎖為 bounded fuzzy-search follow-up，不把 SQL full-scan edit distance 塞進第一刀。
