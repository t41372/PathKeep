...
>
> **2026-04-24 Safari Browser Direct note**：使用者實測發現 `/import` Browser Direct 的 Safari `History.db` 被錯送到 Takeout parser。這輪 stop-ship 修復新增 `inspect_browser_history` / `import_browser_history` backend contract，Browser Direct local DB path 現在會先 staging snapshot + `PRAGMA quick_check`，再分流到 Chromium / Safari parser，並完整接上 preview、canonical URL/visit import、dedupe、import batch revert/restore、source evidence / capability snapshot 與 search projection refresh。公開支援承諾 validated 的 Google Chrome + macOS ChatGPT Atlas + macOS Perplexity Comet + macOS Safari；Safari 不偽造 Chrome-only Favicons / downloads / keyword-search sidecar。
>
> **2026-04-24 ChatGPT Atlas Browser Direct note**：Atlas 已作為 Chromium-family adapter 接進 discovery / backup / Browser Direct import，macOS root 固定為 `~/Library/Application Support/com.openai.atlas/browser-data/host/<profile>`，profile identity 為 `atlas:<raw-profile-dir>`。支援範圍只包含 browser history `History` 與 Chromium sidecars such as `Favicons`；不得導入 Atlas workspace data、chats、tabs、bookmarks 或 suggestions。local validation evidence 只記錄 schema coverage、aggregate counts、time range 與 batch/revert/restore outcome，不記私人 URL / title。

> **2026-04-24 Perplexity Comet Browser Direct note**：Comet 已作為 Chromium-family adapter 接進 discovery / backup / Browser Direct import，macOS root 固定為 `~/Library/Application Support/Comet/<profile>`，profile identity 為 `comet:<raw-profile-dir>`。支援範圍只包含 browser history `History` 與 Chromium sidecars such as `Favicons`；不得導入 Comet AI memory、Perplexity account / workspace data、chats、tabs、bookmarks 或 suggestions。local validation evidence 只記錄 schema coverage、aggregate counts、time range 與 batch/revert/restore outcome，不記私人 URL / title。
>
> **2026-04-28 release blocker browser/scheduler note**：0.1.0 blocker slice 已把 Microsoft Edge / Edge Dev 與 Firefox 提升到 backup + Browser Direct 支援矩陣。Edge 仍走 Chromium parser，但 source profile metadata 必須保留 `Microsoft Edge` / `Microsoft Edge Dev`；Firefox 走 `places.sqlite` history-only parser，不偽造 favicon / downloads / search sidecars。Windows scheduler 也從 manual-review 升級為 app 可 preview / apply / status / remove 的 Task Scheduler flow；Linux scheduler 仍維持 manual-review。
>
...
> **2026-04-23 Core Intelligence DTO boundary note**：`WORK-BE-C` 的下一個 execution slice 已把 `src-tauri/crates/vault-core/src/models/core_intelligence.rs` 拆成 `models/core_intelligence/{mod,shared,requests,reads,analytics,overview,exports,tests}.rs`。Tauri / worker / frontend-facing serde shape、request aliases、section-window enum tags 與 `vault_core::*` re-export surface 都維持不變；下一個 backend support hotspot 改成 `remote.rs`。
...
>
> **2026-05-03 M14 closeout note**：`WORK-M14-A` 已完成並完成供應鏈 remediation；`WORK-M14-C` 隨後恢復符合 trust gate 的 ICU4X NFKC / full-width normalization；`WORK-M14-D` 用官方 OpenCC 1.3.0 字典資產 + repo-owned Rust converter 恢復繁簡 folding；`WORK-M14-B` 完成 bounded alias / fuzzy recall，未新增 third-party dependency。Explorer keyword recall 現在走 deterministic lexical recall v2：ICU4X NFKC + OpenCC-derived `t2s` / `tw2sp` variants + lowercase / compact analyzer、SQLCipher-backed FTS5 unicode61 prefix + trigram projection、CJK 2/3-gram support fields、compact punctuation/space-insensitive recall、BM25 relevance default sort、短別名 `gh` / `yt` / `pr` query expansion、正常 FTS/trigram 無結果時的 bounded Latin typo fallback、以及 opaque relevance cursor。未經批准的 OpenCC binding 依賴已移除；OpenCC native C++ linking 仍需先證明 CMake/C++/CI/package contract；SQL full-scan edit distance、SQLite extension、Jieba、embedding / semantic / vector runtime 均未啟用。
...
- 2026-04-10 boundary / release closeout：`WORK-M4-K` 與 `WORK-M4-L` 已完成。PathKeep 現在正式 shipping macOS Touch ID session unlock、Settings updater review / install surface、`com.yi-ting.pathkeep` clean-break namespace、single-script version bump，以及 release size / code-health artifact docs。plugin sandbox、獨立 enrichment queue family 與 longer-horizon intelligence 已明確移回 `WORK-M5-A` / `WORK-M5-B`；M4 因此 truthfully close out，而不是停在模糊 `[~]`。
