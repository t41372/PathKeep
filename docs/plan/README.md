...
>
> **2026-04-24 Safari Browser Direct note**：使用者實測發現 `/import` Browser Direct 的 Safari `History.db` 被錯送到 Takeout parser。這輪 stop-ship 修復新增 `inspect_browser_history` / `import_browser_history` backend contract，Browser Direct local DB path 現在會先 staging snapshot + `PRAGMA quick_check`，再分流到 Chromium / Safari parser，並完整接上 preview、canonical URL/visit import、dedupe、import batch revert/restore、source evidence / capability snapshot 與 search projection refresh。公開支援仍只承諾 validated 的 Google Chrome + macOS Safari；Safari 不偽造 Chrome-only Favicons / downloads / keyword-search sidecar。
>
