...
>
> **2026-05-03 M14 closeout note**：`WORK-M14-A` 已完成並完成供應鏈 remediation；`WORK-M14-C` 隨後恢復符合 trust gate 的 ICU4X NFKC / full-width normalization。Explorer keyword recall 現在走 deterministic lexical recall v2：ICU4X NFKC + lowercase / compact analyzer、SQLCipher-backed FTS5 unicode61 prefix + trigram projection、CJK 2/3-gram support fields、compact punctuation/space-insensitive recall、BM25 relevance default sort、以及 opaque relevance cursor。未經批准的 OpenCC binding 依賴已移除；繁簡中文 folding 下一步只走官方 OpenCC toolchain proof 或 repo-owned audited implementation。`WORK-M14-B` 已解鎖為 bounded fuzzy-search follow-up，不把 SQL full-scan edit distance 塞進第一刀。
