...
> 2026-04-21 backend follow-up note：Takeout import / batch review boundary的文件尺寸已收斂到 repo 硬限制內，execute path 也已拿掉雙重 inspection/import 解析；`archive/mod.rs` 也已再拆到 `406` 行。剩餘 backend risk 已集中到更深一層的 parser → import/archive collect-then-ingest contract，而不再是 Takeout 或 archive parent module 的 giant-file 本身。

- [ ] **WORK-BE-B** — Import Streaming Contract And Remaining Archive/Takeout Hotspot Split
  - 依賴：`WORK-BE-A`
  - 目標：把 parser → import/archive 的 collect-then-ingest contract 進一步改成可分批/可流式的寫入邊界，並處理這條熱路徑上剩餘的 supporting hotspot（例如 parser-side full-batch materialization 與對應 import/test support file）。
