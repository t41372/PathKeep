...
> 2026-04-23 backend follow-up note：`WORK-BE-C` 已完成 `visit_taxonomy` rename/split、`intelligence/site_dictionary` owner split、`models/core_intelligence` DTO-family split、以及 `remote` bundle/upload/verify owner split。backend 軌道下一個 active focus 已收斂到 `intelligence/mod.rs` 內嵌 regression suite 的後續拆分；`AGENTS.md` 若仍 dirty，視為使用者自有未提交改動，不納入 backend commits。
