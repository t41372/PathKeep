...
> 2026-04-22 backend follow-up note：Takeout import / batch review boundary的文件尺寸已收斂到 repo 硬限制內，execute path 也已拿掉雙重 inspection/import 解析；`archive/mod.rs` 也已再拆到 `406` 行，而且 backup/takeout 在 canonical commit 後已不再保留第二份完整 `ParsedHistory` 只為了寫 cold source evidence。剩餘 backend risk 已更明確地集中到 parser → import/archive collect-then-ingest contract本身，也就是 parser family 仍先 full-batch materialize 再開始 canonical writes。
