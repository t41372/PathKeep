...
> 2026-04-22 backend follow-up note：Takeout import / batch review boundary的文件尺寸已收斂到 repo 硬限制內，execute path 也已拿掉雙重 inspection/import 解析；`archive/mod.rs` 也已再拆到 `406` 行，而且 backup/takeout 在 canonical commit 後已不再保留第二份完整 `ParsedHistory` 只為了寫 cold source evidence。後續又把 Chromium / Firefox / Safari live-backup path 全部切成 streamed canonical ingest，並把 deferred cold source-evidence payload 改成可 spill 到 `staging/source-evidence-spool/` 的 bounded-memory contract；`snapshot_restore` preview 也已改成 direct-count path，不再為 sizing replay 重跑整份 parser batch。再往前推的 follow-up 已把 `browser-history-parser::takeout` 拆成 focused submodules，讓 `vault-core::takeout::payload_import` 直接吃 payload-level streamed canonical rows，把 `inspect_takeout` 切到 streamed preview contract，並把 Takeout import-side source-native evidence 改成 streamed chunk + incremental spill。最新 follow-up 先把 `intelligence_runtime.rs` 正式拆成 queue/claims/job-control/recovery/snapshot/test owners，再把 `intelligence/mod.rs` 的 overview/summary/domain/export、refind/explain、以及 schema/bootstrap/rebuild orchestration 抽成獨立 owner；backend 軌道現在真正剩下的 giant-file 風險已更集中到 `intelligence/mod.rs` 的 structural rebuild internals 與 query/read-model helpers，之後再視情況跟進 `vault-worker/src/intelligence.rs`。

- [ ] **WORK-BE-B** — Core Intelligence Domain Boundary And Worker Follow-Through
  - 依賴：`WORK-BE-A`
  - 目標：把 `intelligence/mod.rs` 剩餘的 structural rebuild internals、query/read-model helper clusters、host-artifact ownership 繼續拆開，並在必要時同步收斂 `vault-worker/src/intelligence.rs` 的 passthrough/orchestration 邊界，避免 queue/runtime、read-model、explain、schema/bootstrap 與 rebuild entrypoint 已拆開後又把複雜度卡在 worker facade 上。
  - 契約：維持現有 Tauri command、worker CLI、serde payload、Core Intelligence query ids / rebuild semantics、`IntelligenceRuntimeSnapshot` 與 background-task off-main-thread 契約穩定；不得因為 giant-file 拆分而重開已接受的 `/intelligence` route / payload grammar。
