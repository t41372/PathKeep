...
> 2026-04-22 backend follow-up note：Takeout import / batch review boundary的文件尺寸已收斂到 repo 硬限制內，execute path 也已拿掉雙重 inspection/import 解析；`archive/mod.rs` 也已再拆到 `406` 行，而且 backup/takeout 在 canonical commit 後已不再保留第二份完整 `ParsedHistory` 只為了寫 cold source evidence。後續又把 Chromium / Firefox / Safari live-backup path 全部切成 streamed canonical ingest，並把 deferred cold source-evidence payload 改成可 spill 到 `staging/source-evidence-spool/` 的 bounded-memory contract；`snapshot_restore` preview 也已改成 direct-count path，不再為 sizing replay 重跑整份 parser batch。最新 follow-up 再把 `browser-history-parser::takeout` 拆成 focused submodules，讓 `vault-core::takeout::payload_import` 直接吃 payload-level streamed canonical rows，把 `inspect_takeout` 切到 streamed preview contract，並進一步把 Takeout import-side source-native evidence 改成 streamed chunk + incremental spill。archive/takeout import boundary 的剩餘 full-batch 風險現在已收斂完成，下一個實際 hotspot 轉向 `intelligence_runtime.rs` 或其他有實測證據的 backend giant-file。

- [ ] **WORK-BE-B** — Intelligence Runtime Queue Boundary And Remaining Backend Hotspot Split
  - 依賴：`WORK-BE-A`
  - 目標：把 `intelligence_runtime.rs` 按 queue writes、lease/recovery、runtime snapshot read model、module/plugin ownership 拆開，為後續 `intelligence/mod.rs` 與 `vault-worker/src/intelligence.rs` 的 giant-file 拆分清出穩定邊界；若在這一步又發現新的實測 backend hotspot，先把證據與 follow-up 補回 planning truth，再決定是否插隊。
  - 契約：維持現有 Tauri command、worker CLI、serde payload、runtime job id / job-type semantics、`IntelligenceRuntimeSnapshot` 結構與 background-task off-main-thread 契約穩定；不得因為拆分而把 queue recovery 或 rebuild control 拉回 UI thread。
