...
- User scoping is part of the Windows compatibility contract: both the `LogonTrigger` and `Principal` carry the current `DOMAIN\USER`, so non-admin installs do not create an all-users logon trigger.
...
- `SCHED-WIN-003` fixed 2026-04-30: real Windows packaged builds could write the XML but fail `schtasks /Create` with `ERROR: Access is denied.` because the XML used an unscoped `<LogonTrigger />` and a principal without `UserId`. The generated XML now binds both trigger and principal to the current Windows user and classifies access-denied results as a dedicated scheduler issue.
