...
- [x] `WORK-ENCRYPT-OOM-UX` (2026-06-28) — encryption-mode overhaul: the backup OOM root-fix, an immediate inline unlock UX with keychain auto-unlock, and an og:image coverage stat. The user enabled database encryption and every backup then failed with `out of memory: Error code 7`; the unlock flow was also unintuitive (a sidebar nudge that routed to Settings) and the existing keychain backend was invisible. Run through the full pipeline (analyze → design → implement + tests → per-block independent review → cross-block integration review → authoritative gate), four blocks plus a supply-chain fix.
  - **A — the SQLITE_NOMEM backup failure, root-caused against the live install** (`1c8e2bb8`): file headers proved it — `rekey_archive` re-encrypted only `archive/history-vault.sqlite` and left `archive/source-evidence.sqlite` (also a canonical encrypted-tier DB) plaintext (`PRAGMA quick_check = ok`, `SQLite format 3\0` header, mtime before the rekey), while `config.archive_mode = Encrypted`. `open_source_evidence_connection` then applied `PRAGMA key` to that plaintext file, so SQLCipher decoded the plaintext header as ciphertext, derived a bogus page size, and aborted with SQLITE_NOMEM on EVERY backup — foreground and scheduled (both share `run_backup_with_progress`). Fix: new `archive/at_rest.rs` detects at-rest mode drift from the 16-byte SQLite magic (no key needed) and converges `source-evidence` in place — `sqlcipher_export` to a temp → atomic rename swap, with `recover_interrupted_rewrite` restoring a swap cut by a crash and scrubbing any leftover (plaintext) backstop so none lingers. It runs DEFENSIVELY at the top of `run_backup_with_progress` (before `open_source_evidence_connection` → fixes manual + scheduled) and PROACTIVELY via a new `reconcile_archive_encryption` Tauri command after unlock. `rekey_archive` now migrates BOTH canonical encrypted-tier DBs in lockstep via `migrate_source_evidence_for_rekey`. The repair is a visible `run_type='rekey', trigger='repair'` run (PME transparency). Scope: MODE drift only — stale-passphrase KEY drift looks identical from the header and is tracked as `WORK-ENCRYPT-KEYDRIFT-A`. Independent review (4 findings) added the crash-recovery (data-safety), scoped the docstrings honestly, and routed the unlock-wiring + audit-label findings to Block B. 100% Rust coverage incl. a `reproduces_oom_then_self_heals_plaintext_source_evidence` regression test.
  - **B — immediate inline unlock + keychain auto-unlock** (`b2a46c61`, designed/authored by Sonnet 4.6): a new `ArchiveUnlockGate` prompts for the passphrase immediately and inline whenever the archive is encrypted + locked (auto-unlock is attempted first, so the gate never flashes when the keychain can unlock silently). A "remember on this device" checkbox — CHECKED by default — stores the key in the OS keychain AND sets `rememberDatabaseKeyInKeyring`, so the next launch auto-unlocks with no prompt; unchecking it clears the keychain entry + flag (no stale-key auto-unlock against the user's opt-out). A backup that fails because the archive is locked opens the gate and RETRIES after unlock instead of raising the red failure toast. After any successful unlock (manual, saved-password, or auto) the app calls `reconcile_archive_encryption` fire-and-forget to self-heal a drifted at-rest state, and that automatic repair reads honestly in Audit ("Re-encrypt · Automatic Repair") via a new `audit.automaticRepair` trigger label. Settings gains an auto-saving keychain toggle (disabled honestly when the keychain is unavailable — no dishonest "Saved"); the old multi-button unlock panel and the `/security#unlock-archive` detour are retired. Independent code + product/UX review fixed H1 (focus-trap + keyring-failure coverage), M1 (unchecked-remember no-op → keychain/flag desync), M2 (toggle not truly disabled when keychain unavailable), and five LOW polish items. All copy en/zh-CN/zh-TW; 100% JS coverage; ⌘K inert while the gate is up; reduced-motion-safe.
  - **C — og:image coverage stat** (backend `1c8e2bb8` / frontend `b2a46c61`): Settings → Link previews now shows what share of web pages carry a preview image. Independent review caught two honesty bugs, both fixed: the denominator is HTTPS-only (the fetcher short-circuits `http://` to `parse_error`, so counting http pages understated coverage) and uses `COUNT(DISTINCT url)` (the `urls` table has no unique constraint on `url`, so a multi-profile archive double-counted). Byte-range prefix on `idx_urls_url` keeps it index-seekable; computed on-demand off the UI thread with honest loading / error / not-fetched states (never a misleading flat 0%).
  - **D — docs / backlog / memory**: documented the encryption-at-rest posture in `docs/architecture/data-model.md` (archive + source-evidence encrypted; derived sidecars plaintext by design; the lockstep rekey + auto-repair). Backlogged `WORK-ENCRYPT-AT-REST-ALL` (evaluate encrypting the derived search/intelligence/agent sidecars — they still hold URLs/titles/summaries/chat in plaintext — perf + design first, per the user) and `WORK-ENCRYPT-KEYDRIFT-A`. Refreshed the `project_backup_fda_and_transparency` memory with this third backup-failure mode.
  - **Cross-block integration review**: empirically verified (compiled, ran targeted Rust + JS suites, i18n + coverage) — coherent, correct, ready to commit, no blocking defects; the headline fix proven end-to-end in both the defensive (backup) and proactive (on-unlock) paths. While running the full gate I also closed 3 pre-existing staging-fallback coverage gaps left by the Jun-27 commits (`72f0dd7d`/`73c1af80`).
  - **Supply chain** (`855c75ba`): the full `audit:rust` surfaced two newly-published 2026 RustSec advisories against pre-existing transitive deps (RUSTSEC-2026-0185 quinn-proto — unreachable phantom lock entry; RUSTSEC-2026-0186 memmap2 — reachable via candle). Per the user's call, fixed by Cargo.lock-only patch bumps (memmap2 0.9.10→0.9.11, quinn-proto 0.11.14→0.11.15) within the existing ranges, rather than an allowlist.
  - Gate (authoritative, all green): `cargo fmt --all` / `clippy --workspace --all-targets --all-features -D warnings` / `deny:rust` / `audit:rust` (green after the patch bumps) / Rust coverage 100% (43,719 lines, skipping the known macOS `/dev/shm` test) / typecheck / lint / `format:check` / `check:i18n` 100% (3228 keys × 3) / `coverage:js` global 100%. Design + copy by Sonnet 4.6. Real-machine acceptance (rebuild `.app` → user unlocks → auto-repair flips the source-evidence header → backup completes with no OOM) in progress.

- [x] `WORK-NOTESEARCH-EMBED-EXPOSE` (2026-06-28) — note/tag keyword search, the two-tier embedding exposure + recovery, and a test-methodology reform. On the v0.3.0 build the user reported two "touch-it-and-it-breaks" defects and one methodology challenge: (1) keyword search couldn't find note content (with or without the `note:` prefix); (2) embedding could be invoked but a single interruption wedged the queue forever; (3) "why did these break when the tests passed — are our tests incomplete?". Run through the full pipeline (analyze → design → implement + behavioral tests → per-block independent review → cross-block integration review → authoritative gate), four blocks.
  - **E — notes & tags are keyword-searchable** (`774cd0e3`): root cause — notes (`url_annotations`) and tags (`url_tags`) were never projected into the keyword FTS document (which indexed only url/title/search_terms/enrichment_text), and the `note:`/`tag:` operators ran as post-FTS filters, so a page whose only match was a saved note never reached the filter. Projected `notes_text` + `tags_text` into `search_documents` and the `history_search_terms` term FTS (mirroring the `enrichment_text` precedent), bumped `SEARCH_PROJECTION_SCHEMA_VERSION` 3→4 (drops + rebuilds off-thread on next open), and folded notes/tags into the CJK `cjk_grams` + trigram planes so a contiguous CJK note run is searchable, not just latin tokens. A refresh hook on `set_notes`/`replace_tags` updates that URL's row + re-mirrors the term FTS so an edited note is findable immediately. bm25 extended to the 10 columns. Independent review fixed the CJK gap (HIGH — the user's exact case). Behavioral tests (the assertions that were missing): set a note → PLAIN keyword finds the page; same for a tag; same for a CJK note and CJK tag; edit a note → new word found, old word drops via the hook.
  - **F (backend) — static embedding tier as default + recoverable, honest builds** (`8ed7bf33`): the two-tier design (static base + optional external/qwen3) was built but never exposed — the static engine was only selected when `base_url == "static:in-app"`, nothing auto-created it, and the UI never offered it, so an index build used only the external tier, which (a) wrote 0 vectors while reporting "indexed" and (b) couldn't resume after one interruption (the cursor persisted only after a successful chunk → an infinite stale→running→stale cycle). And the real embedding engines were `#[cfg(not(any(test, coverage)))]` — compiled OUT of the coverage binary and replaced by always-succeed stubs, which is exactly why "100% coverage" never caught the 0-vector bug. F1: hydrate a non-deletable `static-in-app` provider into the config and make it the default when none is validly selected (`merge_embedding_providers` / `normalize_app_config`), plus `StaticEmbeddingStatus` + `semantic_vector_count` read models. F2: checkpoint the last-durable cursor on the FAILURE path (a reclaim resumes from real progress, never origin), bound attempts to a terminal `error_code` instead of cycling forever, reset attempts on durable progress so an interrupted build keeps resuming, and add `clear_index_jobs` + `reset_ai_index_build` to clear a wedged job. F3: validate each batch (non-empty, expected dim) before counting, and surface `IndexVectorsMissing` (the real `.pkvec` count) instead of "indexed" with 0 vectors. F4: the static compute path (tokenize → table lookup → mean-pool → L2-norm) is now compiled ALWAYS — only the disk model-load stays behind a seam — so the gate runs the REAL engine and asserts deterministic non-zero vectors. Integration-review fix: reset is now a FULL rebuild — incremental dedup keys `needs_embedding` on the SQLite `ai_embeddings` rows, not the `.pkvec`, so an incremental reset would skip a degraded 0-vector index and leave it broken; `reset_ai_index_build` coerces `full_rebuild=true` so a 0-vector index genuinely self-heals (the worker test asserts the staged job is a full rebuild). New commands `reset_ai_index_build` + `download_static_embedding_model` wired through commands/worker_bridge/dispatch/generate_handler.
  - **F (frontend) — the built-in static vector model + honest recovery** (`3ed3adcc`, copy by Sonnet 4.6): Settings → AI providers now shows a StaticEmbeddingPanel (the Static (in-app) tier is visible, selectable, recommended as default) with a consent-gated model download driven by live per-file progress on `pathkeep://model-download-progress` (new `lib/ipc/model-download`, with cancel); the built-in static provider renders locked in the editor (fields disabled, no Remove, no API-key field, "built-in" badge); the degraded `IndexVectorsMissing` state (real `semantic_vector_count`) surfaces via the note-codes localizer (3 locales) instead of claiming the index is built; an IndexResetButton clears a wedged/degraded build and rebuilds. Integration-review fix: the reset confirm prompt + queued toast now honestly say it rebuilds the index from scratch (re-embeds every page) to match the backend full-rebuild coercion, replacing the old "pages missing a vector" incremental wording that didn't heal a 0-vector index; `resetAiIndexBuild` sends the required `AiIndexRequest` (the wire contract has no serde default for its bools), with a backend-client test asserting the real args.
  - **G — test-methodology reform** (`quality-matrix.md` + `BACKLOG.md`): codified the "coverage ≠ behavior" behavioral-assertion rule (5 points — coverage is necessary but not sufficient; every user-visible feature needs ≥1 end-to-end assertion from the real entry point; no production I/O path may be `cfg`-compiled out of the coverage binary, seam at the transport endpoint; no coverage-theater — `let _ =` swallows, fire-and-forget walks, render-the-string; new-feature review must ask "is there a test that asserts the expected result from the real surface, and is the real I/O compiled out?"). Backlogged `WORK-TEST-REAL-IO-A` (external/candle transport behind an injectable seam + a fake-HTTP `/v1/embeddings` integration test for mid-batch 500 / empty `data[]` / timeout; `dispatch_for_coverage`'s fire-and-forget walk → per-command contract assertions).
  - **Cross-block integration review**: a read-only subagent over the merged diff confirmed the FE↔backend contract aligns (camelCase serde, the note-code enum round-trips, every new command has handler + dispatch + client, i18n parity holds) and that the headline behaviors are proven by real tests (CJK-note keyword recall; static-tier build → non-zero vectors → semantic search finds the seeded page; zero-vector build fails honestly; resume after interrupt). It caught and we fixed 1 HIGH + 1 MEDIUM: HIGH — `resetAiIndexBuild` was invoked with no `request`, so the reset button errored on both transports (fixed: FE sends the request + backend coerces a full rebuild + a real-args test); MEDIUM — the reset self-heal copy didn't match the SQLite-keyed dedup (fixed together with the HIGH: reset is now a full rebuild and the copy is honest).
  - Gate (authoritative, all green): `cargo fmt --all` / `clippy --workspace --all-targets --all-features -D warnings` / `deny:rust` / `audit:rust` (only the known allowed transitive advisories) / **Rust coverage 100% (44,217 lines / 2,484 functions, skipping the known macOS `/dev/shm` test)** / typecheck / lint / `format:check` / `check:i18n` 100% (3250 keys × 3) / `coverage:js` global 100% (3155 tests). Real-machine acceptance (rebuild `.app` → Static (in-app) visible + default → download → index builds non-zero vectors → CJK note keyword search finds the page → Reset recovers a wedged build) pending user confirmation.

- [x] `WORK-ACTIVITY-VECTORMODEL-REDESIGN` (2026-06-29) — the task-queue page redesign, the vector-model two-tier settings reframe, and honest download/index progress. On the v0.3.0 build the user reported six things: (1) the built-in static embedding tier looked like "an external alternative" rather than the always-on base layer of the two-tier system; (2) it showed dimension 1536 (the model is 256); (3) the model download had no progress bar and stuck on "downloading" after finishing; (4) index-build progress wasn't visible; (5) all long tasks can be interrupted by quitting — they must preserve progress, warn, or run in background; (6) the settings screen and the task-queue page were "maximally complex while conveying minimal information — completely redo the task-queue page." Run through the full pipeline (analyze → Sonnet design → three blocks each implemented + independently reviewed + fixed → cross-block integration review → authoritative gate); the user chose "unify ALL background work" as the Activity-center scope.
  - **A — backend honest-progress contract** (`eb254c70`): index build — `IndexBackfillCursor.scan_target` (max candidate history_id captured once at a fresh build's true start, preserved on resume) + `AiQueueJob.{progressEmbedded,progressScanned,progressScanTarget}` (parsed from the index job payload) → a determinate scan-progress bar + an honest "{n} pages embedded" count, no fabricated %. `StaticEmbeddingStatus.{dimensions=256 (STATIC_EMBEDDING_DIM), modelSizeBytes}` so the FE shows the real 256 dim. Real model-download byte progress — a new `FileProgress` event + `ModelDownloadProgress::file_progress` (default no-op); `ensure_model_downloaded` now streams via a pure, always-compiled `stream_to_file` (chunked read/write + throttled progress + cancel-removes-partial via an RAII guard), with the ONLY cfg-gated seam being the socket (`open_download_stream`, ureq promoted from a hf-hub transitive to a direct dep — no new crate in the graph); FileStarted carries the real content-length, not 0. Review fixes: the enum struct-variant fields wire camelCase via per-variant `rename_all` (container-level does not cascade — same fix as `AiChatStreamChunk`), pinned by a wire-shape test; the RAII guard scrubs the partial on every error path. 100% Rust coverage (44,324 lines); the streaming loop is in the measured build, only the socket is seamed (the coverage≠behavior rule).
  - **B — settings vector-model two-tier reframe** (`03f02cac`, copy by Sonnet 4.6): the static tier is removed from the editable provider list (it stays in config for the backend) and given a dedicated BASE TIER panel (built-in, on-device — no server/no key; **lower precision** stated plainly; 256-dim, real size) above an UPGRADE TIER for optional external models. Honest download — a real byte progress bar (clamped) or an honest indeterminate state when Content-Length is unknown, the current file, and a "restarts the current file if you quit (finished files are kept)" note; the shared `useModelDownloadProgress` hook (reused by Block C) sets `ready` immediately on `done` (fixing the stuck-on-downloading bug). Review fixes: M1 (a retry after a mid-download failure no longer flashes a stale "Download failed" — a restartNonce resets the hook's phase + the hook clears its error on fileStarted/done), M2 (tier labels are real `<h5>` headings, not aria-hidden, and the external list no longer emits an empty `<h3>`), aria-valuenow clamp, the BASE TIER block gated on staticEmbedding, a static fill under reduced motion. 100% JS coverage.
  - **C — Activity-center page redesign** (`95ae1d89` + integration fix `b342303c`): the 7-section monster page becomes a 4-zone honest Activity center (Needs attention / Running now [real progress + interruption badges] / Background features chips / Recent). A pure `activity-adapter` maps the AI queue + intelligence runtime + archive import/backup into a unified `Activity[]` (filters chat jobs, clamps progress, encodes resume behavior per type). Route label Jobs→Activity (path `/jobs` unchanged). The implementation subagent ran in a STALE worktree (before A/B) and PORTED its output — confirmed it consumes the real contracts (progressScanned/ScanTarget + the real hook), with the committed A/B files untouched. The independent review caught that the redesign **shipped with zero CSS** (functionally wired + tested but unstyled) plus four honesty gaps — all fixed: wrote `activity.css` (613 lines, paper aesthetic, tone rails, pulsing dot, progress visuals, skeleton, all reduced-motion-gated), per-kind progress labels via a `labelKind` discriminant (a runtime job was rendering "Rebuilding visit stats pages embedded"; import/backup showed no count), a wired `runBackupNow()` action on failed/stale backups (was a no-op link to `/jobs`), and an aria-live announcer.
  - **Cross-block integration review**: the contract aligns end-to-end (the camelCase wire fix holds, every consumed field is emitted, the shared hook is reused within its contract, coverage is honest), and 5 of the 6 complaints are fully resolved. It found one HIGH: a RUNNING import/backup was labeled "Safe to close · resumes where it left off" but neither resumes (the same task self-reports "Can't resume" once interrupted, and recovery is a fresh re-run) — fixed with a new `restart-whole` resumability (data safe, task restarts; warning tone) and a header "safe to close" that now requires EVERY running task (incl. the download) to be resume-safe; plus a MEDIUM (the header read "All caught up" while queued-but-not-running unpaused work showed in the Running zone — new `headerSummaryQueued` branch). `b342303c` closed both at 100% JS coverage (3,267 tests). Deferred (noted, latent): a cross-surface download cancel maps to "failed" on the Activity row (only if both surfaces ever co-mount, which routing prevents); the byte bar under-counts if a surface mounts mid-download (cosmetic).
  - Gate (authoritative, all green): Rust unchanged since the Block A validation (`fmt --all` / clippy `-D warnings` / `deny` / `audit` / coverage 100% / 44,324 lines); FE typecheck / lint / `format:check` / `check:i18n` 100% (3270 keys × 3) / `coverage:js` global 100% (326 files / 3,267 tests). Design + copy by Sonnet 4.6. The independent reviews (zero-CSS, the dishonest progress labels, the resumability mislabel) are the clearest demonstration yet of the coverage≠behavior rule — the gate passed green while all of these hid; only human-judgment review caught them. Infra note: several long FE subagents stalled on this machine's stream watchdog (partial work was salvageable, typically ~90% complete), and one Block C subagent ran in a stale worktree requiring a careful port. Real-machine acceptance (rebuild `.app` → Settings shows the two-tier vector model with the static base tier, 256 dim, and a real download progress bar → the Activity page shows the 4 zones, real index progress, and honest interruption badges) pending user confirmation.

- [x] `WORK-ACTIVITY-ANIM-FLASH` (2026-06-29) — the indeterminate progress-bar animation, app-wide, and the Activity page's every-few-seconds flash + full state reset. On the rebuilt v0.3.0 the user reported two things about the Activity page: (1) the blue indeterminate bar — and **every animation like it in the app, e.g. the backup progress bar on startup** — crawls left→right, stops around the middle, vanishes, and reappears on the right; it **never touches the leftmost edge**: "obviously hand-rolled, badly written, and tells the user nothing about progress"; (2) the Activity page **flashes and fully refreshes every few seconds, resetting all page state to its initial value** (a pre-existing latent). Run through the full pipeline (analyze → Sonnet animation design → two independently-reviewed blocks → integration review → authoritative gate). The two blocks touch disjoint files (animation = CSS + the 5 progress components; flash = `jobs/index.tsx` logic).
  - **Animation (Sonnet design + a completeness pass)**: root cause — the app had **six independently hand-rolled indeterminate sweeps**, all defective. The worst, the backup startup strip (`background-progress-glide`), held `translateX(180%)` from the `50%`→`100%` keyframes, **freezing for the entire second half of every cycle** (exactly the "stops in the middle, vanishes, reappears on the right" the user saw). The others (`activity-shimmer` / `task-progress-sweep` / `sidebar-track-move` / `busy-overlay-progress-sweep`) shared family flaws: `ease-in-out` stalling at both ends (reads as "stuck"), bars too narrow to cleanly touch the left edge, and arbitrary translate ranges. Fix: converge onto ONE canonical, transform-only `@keyframes pk-indeterminate-sweep` + `.pk-indeterminate-bar` (in `progress.css`: 30%-wide bar, `linear`, `translateX(-100%)→333.333%` — `333.333% × 0.30 = 100%` exact, so it touches both edges with no freeze/stall and the bar fully clears the right edge before the loop restarts). All five bespoke keyframes deleted; three surfaces (`progress.css`, `sidebar.css`, `page-layout.css`) that previously animated even under `prefers-reduced-motion` gained the missing `no-preference` guard (an accessibility fix). Completeness: a sixth surface — the Settings model-download bar, previously an in-place `animate-pulse w-1/3` — was also converged, so the model download now looks identical in Settings and on the Activity page. Every determinate (real-%) path was left untouched.
  - **Flash / state reset (`jobs/index.tsx`)**: root cause — the page gated its full-page `<ActivitySkeleton/>` on `runtimeStatus.loading`, which the shell re-flips to `true` on every 3s background poll (`shell-runtime-status.ts:101`); so every tick the entire zone subtree unmounted and remounted — the visible flash, and why every child's local state (Recent expand toggle, scroll) reset to initial. Fix: the skeleton now shows only on the genuine first load (`!error && aiQueue === null`); once data is present, a background refresh keeps the page mounted and the zones reconcile in place. `aiQueue` and `intelligence` are populated/cleared together by the shell's single `Promise.all` (both null on reset/error), so gating on `aiQueue` alone is sufficient (coupling verified in review).
  - **Independent reviews + fixes**: the animation block's independent (adversarial) reviewer returned **ship**, with 1 MEDIUM (test 1b passes on both old and new gate, so it documents first-load behavior but can't catch a regression — annotated to point at test 1c as the real guard) and 3 LOW (`333%`→`333.333%` for exact edge contact, applied; sidebar reduced-motion static fill has the weakest disambiguation context — accepted house-style tradeoff, left; `aria-busy` on `role="progressbar"` redundant — pre-existing, not introduced, left). The flash fix is **regression-proven**: on the old `runtimeStatus.loading || …` gate, test 1c renders the skeleton (fails); on the new gate it renders content (passes).
  - Tests: `jobs/index.test.tsx` gains a first-load-skeleton test and a background-poll-no-flash regression test; all 6 affected component suites green (98). Docs: `ui-review-guardrails.md` adds reviewer question #9 ("is the progress/loading animation honest?") + a progress-bar red-line (canonical sweep only — no hand-rolled keyframes; transform-only; honest determinate-vs-indeterminate; reduced-motion-gated honest static fallback).
  - Gate (FE authoritative, all green; no Rust files changed): typecheck / lint (`--max-warnings 0`) / `format:check` / `check:i18n` 100% (3270 keys × 3, no new copy) / `coverage:js` global 100% (326 files). Animation feel by Sonnet 4.6 ([[feedback_ui_copy_model]]); fluidity-is-a-hard-requirement honored ([[feedback_frontend_performance]] — no freeze, no flash). Deferred (noted, non-blocking): `ShellDataContext.Provider`'s value is an inline literal (fresh identity + re-wrapped callbacks every render) — an app-wide re-render smell worth a future memoize, but it only causes re-renders (not unmounts) so it is not part of this bug. Real-machine acceptance (rebuild `.app` → the progress bars sweep smoothly edge-to-edge with no mid-track freeze → the Activity page no longer flashes/resets every few seconds) pending user confirmation.

- [x] `WORK-INDEX-RESUME-PROGRESS` (2026-06-29) — the interrupted index build that could never resume (a queue wedge), and a progress bar that "has nothing to do with progress". On the rebuilt v0.3.0 the user reported the smart-search index stuck at "73,536 pages embedded" for a long time, never resuming after an interruption (all embedding models configured correctly), and a determinate bar disconnected from real work. Diagnosed against the LIVE install (`history-intelligence.sqlite` `ai_jobs` + `ai_index_ledger`, `rust.log`). Run through the full pipeline (diagnose → design → implement + tests → independent adversarial review → fix the review's HIGH → authoritative gate).
  - **The wedge — root cause**: the index job sat in `state=running` with a 5-day-stale lease — the worker that owned it (PID 21668) had died, but the cursor was intact (`next_history_id=73611`, `embedded_so_far=73536`). The recovery path (`mark_stale_jobs` → reclaim → resume from cursor) was never invoked because the background drain pool is spawned only when there is "recoverable work", and that signal was the read-model `queued` count — which counts `queued`/`paused`/`stale` but NEVER a job orphaned in `running`. So `queued=0` → no drain → `claim_next_ai_job` never called → the job stayed wedged across every launch. The "resumes next time" promise was never kept.
  - **The wedge — fix** (vault-core `ai_queue.rs` + `ai/read_model.rs`, vault-worker `intelligence/ai_queue.rs`): a read-only `count_recoverable_ai_jobs` that ALSO counts orphaned-running jobs (mirroring `mark_stale_jobs`' two orphan predicates byte-for-byte, so the count equals what the next claim will actually recover) + a `recoverable_ai_jobs` wrapper; `load_ai_queue` now kicks the drain on that instead of `status.queued`. The periodic status poll self-heals the wedge — including the user's currently-stuck job, which resumes from 73,611 (no re-embedding) the moment the rebuilt app runs. A regression test reproduces the exact deadlock (the orphan is invisible to `queued` but visible to the new signal, then re-claims). Verified: a healthy heartbeating job is NOT counted (no spurious drains); `maybe_spawn_worker_pool` caps concurrency.
  - **Honest progress bar** (user chose a real percentage): a new `embed_target` (total candidate pages) on the cursor, riding the same rails as `scan_target` (captured at the build's true start, preserved on resume, `serde(default)` 0 for back-compat). The Activity bar fills by `embedded / embed_target` with a "{embedded} / {total} pages embedded" label (FE by Sonnet 4.6, 3 locales, locale-grouped counts). **The independent review caught a HIGH**: `embedded_so_far` was per-session (reset to 0 each run) while `embed_target` is whole-corpus, so on the resume path the wedge-fix enables, the bar would collapse to ~0% and finish under 100%. Fixed by seeding the running total from the cursor's `embedded_so_far` (threaded through `build_ai_index_with_control` / `run_embedding_backfill` + a worker `index_start_embedded`), so the count is CUMULATIVE across reclaims — proven by a regression test (resume seeds 2, embeds 3, asserts 5 not 3). Also fixed the review's MEDIUM: the denominator is now "candidate pages NOT yet embedded for this provider" (anti-join) — a full rebuild's wipe clears `ai_embeddings` first, so it counts every candidate, while an incremental pass counts only the new pages; honest for both. FE falls back to the scan ratio when `embed_target` is unknown (legacy cursors still show a bar).
  - **Independent review (Opus, adversarial)**: the orphan-recovery half was rated ship-ready (predicate mirroring exact, recovery chain fully traced, concurrency capped, no spurious kicks). Its HIGH (cumulative numerator) + MEDIUM (incremental denominator) were fixed; LOWs addressed — a `stop_requested` orphan-branch test added (coverage≠behavior), the read-model's second per-poll intelligence connection noted as a deferred background-thread micro-opt, the urls/source_profiles JOIN omission consciously matched to the existing `max_candidate_history_id` convention.
  - Gate (authoritative, all green): `cargo fmt --all` / clippy `-D warnings` / vault-core + vault-worker lib suites green / `coverage:rust` 100% on new lines (skipping the known macOS `/dev/shm` test) / FE typecheck / lint / `format:check` / `check:i18n` 100% (3271 keys × 3) / `coverage:js` global 100% (326 files / 3,274 tests). Design + copy by Sonnet 4.6. Memory `project_activity_page_and_vectormodel_ui` updated (the queue-wedge signal + cumulative numerator + anti-join denominator). Real-machine acceptance (rebuild `.app` → the stuck index resumes on its own → the bar shows a real "embedded / total" %) pending user confirmation.

- [x] `WORK-ARCHIVE-DURABILITY` (2026-06-30) — backup-core crash-safety hardening + launch auto-heal + a calm snapshot-recovery GUI + a test-methodology sweep. Triggered by a real user incident: an impossible on-disk state (both canonical DBs SQLCipher-encrypted, but `config.json` said `Plaintext`, and history-vault's salt was zeroed) → every launch hit `SQLITE_NOTADB` → the whole app bricked at a dead error page — and a 100%-green gate never caught it. User mandate: backup is the CORE responsibility; minimize by MECHANISM any chance of damaging data; and even when data does blow up, the user must get a comfortable in-app path back to a snapshot — never "your DB is broken and I don't know what to do." User chose "all of Tracks 1–4 at once." Run entirely through the mandated non-negotiable loop ([[feedback_subagent_implement_review_loop]]): the main agent never writes the important code — a clean-context Opus subagent implements, an INDEPENDENT Opus subagent adversarially reviews, an Opus subagent fixes, looping review→fix until a fresh independent reviewer finds no MAJOR issue; the main agent only orchestrates, gates, and commits. An adversarial Opus audit cluster (48 scenarios → 27 confirmed → 5 CRITICAL) seeded the roadmap (`docs/review/2026-06-30-data-integrity-audit/{findings,remediation-plan}.md`). Every independent review round caught a real defect — the loop's value proven repeatedly.
  - **Phase A — foundation primitives** (`8926acb8`, `f6d74cc5`): `durable_io` (atomic write → `F_FULLFSYNC` → rename → parent-dir fsync; ENOTSUP fallback for exFAT/SMB caught in review); `ArchiveWriteLock` (cross-process `flock`, process-reentrant single-fd manager — review caught a backup→reconcile self-deadlock + a last-drop spurious-None race); `fault_inject` (named crash-window checkpoints, production-inert, must-fire guard). `save_config` switched to the durable path; config.json tightened 0644→0600.
  - **Phase B — every destructive write hardened** (Track 1): **rekey** (`69bb3c28`) reordered to lock → verified snapshot → export temp → keyed quick_check → durable swap + sidecar scrub → source-evidence lockstep migrate → **config written LAST** (CRIT-2); **backup** (`fcd334eb`) takes the write lock, scheduled `due_only` defers (CRIT-1/4/5 cross-process); **import + the crash-recovery engine** (`32366b99`) — `apply_import` became a journaled three-phase commit (durable `.pk-import-journal.json` before the first rename; both DBs as one commit unit; scrub the target's pre-existing `-wal` so a prior crash's WAL can't replay = CRIT-3; the old DB's own hot `-wal` rides with its `.bak`; config forced to the installed DB's real header; marker cleared only after config is durable; rollback-from-`.bak` on error; a fail-closed guard that leaves the marker + writes no config rather than silent drift). `recover_interrupted_import` wired recover-first into every destructive op. Three independent review rounds caught: two HIGHs (non-atomic two-DB swap w/o rollback; config-skip mode drift), then a HIGH the recovery itself introduced (rekey on an unrecovered half-import → permanent drift brick).
  - **Lock-completion** (`fd50bd9f`): `ArchiveOpGate` — a process-global, NON-reentrant, keyed-by-archive in-process gate layered above the reentrant cross-process flock, so two same-process top-level ops mutually exclude (closing CRIT-5's same-process trigger) while nested helpers can't self-deadlock (type-safe public-vs-`_locked` variants). reconcile / retention-prune / snapshot-restore brought under the lock + recover-first. Condvar with while-predicate + notify_all (no lost wakeup); flock releases before the gate. Independent concurrency review confirmed no top-level→top-level edge → no self-deadlock.
  - **Phase C — launch auto-heal** (`b60a6eb7` + fixes): `recover_archive_on_launch` runs before the archive opens — a durable `.pk-rekey-journal.json` + `recover_interrupted_rekey` finally make an interrupted rekey DETECTABLE, and a config↔file at-rest drift is healed (header reads only, no key, no DB open) so THE incident (encrypted files under a Plaintext config → NOTADB) turns into a graceful unlock prompt, or a structured `LaunchRecovery::Unrecoverable` report for the GUI. **Launch never freezes** (hard rule): the healthy path takes NO lock (a cheap `launch_is_provably_healthy` pre-check) so a healthy start is never blocked behind a running scheduled backup; only a marker/drift escalates to the gated blocking recovery. Independent review's soundness proof: the fast path returns Healthy only when the locked path would no-op. Two MEDIUM fixes (the launch-block; a decrypt-direction fail-closed).
  - **Phase D — snapshot + recovery GUI** (Track 3): **backend** (`67adb55c`) `run_full_archive_snapshot_restore` automates the once-manual full-archive restore — path-traversal-guarded, keyed-quick_check-verified BEFORE touching the live archive, quarantines the broken state (move, never delete), durable install, config reconciled last, and it is itself a crash-recoverable commit unit (a `.pk-restore-journal.json` + `recover_interrupted_restore` complete-or-rollback on next launch, never boot an empty archive — the HIGH the independent review caught). `list_recovery_snapshots` (keyless date/size/verified/source-op metadata). Verified safety snapshots before reconcile/import rewrites; retention keeps the last-good. **FE** (`a59d72ad`) the guided full-screen recovery screen (newest-verified prominent + one-click restore behind a destructive confirm; full list secondary) chosen by the user + a proactive Settings entry; startup detection routes on the recovery report; desktop Tauri command wiring; Preview deliberately omitted (it opens the un-openable live archive). **Encrypted-user completeness** (`85603df0`) closed the review's gap so "never a dead end" holds for encrypted users too — an unlock-gate "Can't unlock? Recover from a snapshot" escape hatch, honest "Encrypted · needs your key" labeling (a new keyless `encrypted` flag), a key-entry field threading an explicit key to the restore (the key never leaks — verified every hop), always a "Reveal logs" forward path, and deterministic modal focus containment (a focus-trap escape was the review's blocker).
  - **Phase E — test methodology** (`4c99ee69`, Track 4): the answer to "why didn't we catch this." A `config↔disk` invariant checker asserted after every archive-mutation test, with proof tests that reproduce the incident's REAL on-disk shape and FAIL if the checker regresses to a no-op or the config-LAST ordering is reverted; deterministic per-checkpoint crash-torture + a same-process serialization test (consistent-or-fail-closed at every checkpoint); real-SQLCipher encrypt→kill→reopen round-trips using the real `load_config` path; `quality-matrix.md` gained 4 blocking rules (durability, config↔disk consistency, crash-window coverage, no production I/O cfg-compiled-out of the coverage binary); and a pre-existing global-panic-hook suite flake fixed (thread-local opt-in + forward foreign panics).
  - **Outcome**: all 5 CRITICALs closed; the exact incident now auto-heals on launch (or offers the one-click restore GUI); the restore path can never boot an empty archive; every destructive write is atomic + durable + serialized cross- AND same-process; and the suite now behaviorally catches the incident class instead of passing it green. Gate on every commit: `cargo fmt --all` / clippy `-D warnings` / vault-core + vault-worker suites (the macOS `/dev/shm` cross-fs migration test is the only known-fail, a Linux-only gate limitation) / desktop fmt+clippy / FE typecheck+lint+`check:i18n` 100% (3329 keys ×3) + `coverage:js` 100% (333 files / 3407 tests) / repo-wide prettier — all green. Carry-ins (recorded in `remediation-plan.md`): the encrypted-snapshot retention last-good guard is size-heuristic-based (keyed verify or keep-N follow-up); the Phase-C discriminant-vs-on-disk-config LOW; a Phase-E source-evidence-specific assertion nit; in-place wrong-key correction as a UX follow-up. **Real-machine acceptance pending** (user rebuilds the `.app` and confirms the whole hardening + recovery GUI on their real archive — the Jun-28 plaintext snapshot restore that recovered their 388,623 visits stands).

- [x] `WORK-UPGRADE-V020-SAFETY` (2026-07-01) — verify + smooth the v0.2.0 → v0.3.0 upgrade path. After the data-integrity initiative closed, the maintainer (having just personally hit a rekey-zeroed-salt disaster on the dev machine — traced to the OLD un-hardened rekey re-triggering after a recovery, NOT the new code, which didn't exist until hours later) asked to confirm real v0.2.0 users can upgrade without hitting problems. Data-loss was not a concern for the dev archive (re-importable from Chrome); the ship-blocker was upgrade safety. Run through the loop.
  - **Proven data-safe** (`d6c2f0d8`): a real end-to-end regression test builds an AUTHENTIC shipped-v0.2.0 install (canonical schema capped at v10 — proven byte-identical migrations 001–010 between tag `v0.2.0` and HEAD; search projection v2; a v0.2.0 config.json with the since-removed `remoteBackup` block) and drives the REAL production launch/open path (`recover_archive_on_launch` → `ensure_archive_initialized`). Asserts: 011–015 apply (max v15), archive opens plaintext with `integrity_check == ok` (no NOTADB), data intact (counts preserved, `015` backfill fills `registrable_domain` with zero NULLs/row-loss — all migrations are `CREATE … IF NOT EXISTS` + one nullable `ALTER`, zero destructive rewrites), `recover_archive_on_launch == Healthy` (no misfire on a legit old archive — reads only markers+header, never a v0.3-only table), search reprojects v2→v4, `load_config` yields `archive_mode=Plaintext` (container-level serde-default → the config↔disk drift that caused the incident is structurally avoided on a fresh upgrade) + `check_config_disk_consistency` passes. The audit confirmed migrations are non-destructive + resumable, config forward-compat is clean (no `deny_unknown_fields`; profile selections + `capture_favicons` preserved), and the agent DB is brand-new in 0.3.0 (nothing to migrate).
  - **First-upgrade progress** (backend `9ce4a98a` + FE `411fdd1d`): the one MEDIUM the audit found — on a 14.4M-row archive the first post-upgrade launch does minutes of one-time work (index builds + `015` backfill + full search reprojection) inside `ensure_archive_initialized` with no progress (an opaque busy overlay, against the "no silent multi-minute wait" rule). Backend: `assess_archive_upgrade` (O(1) at steady state — never scans on a normal launch) + `ensure_archive_initialized_with_progress` threading a callback through the migrations/backfill/reprojection (OBSERVATION-ONLY — migration SQL/ordering/ledger/batch-size/commit-boundaries byte-unchanged, every canonical-DB report fires OUTSIDE the write transaction, resumability intact) + a `pathkeep://archive-upgrade` Tauri event. FE: a calm full-screen "Upgrading your archive…" screen (determinate bars for the backfill/reprojection, canonical indeterminate sweep for the opaque index-build steps, info line for the lazy intelligence phase, honest "Step N of M" over streamed phases, reassurance that it's one-time + quit-and-resume) that sits BEHIND recovery/unlock in bootstrap precedence, dismisses ONLY when `initialize_archive` resolves (never on a `done` tick alone — no half-migrated shell), and never renders on a normal launch. Each block: implement → independent review → fix. Reviews caught: an O(corpus)-per-launch scan in `assess` (→ O(1)), an Intelligence-phase FE trap (`streamed:false`), a dishonest "Step N of M". Gate all-green (Rust: upgrade 7 + archive 436 + worker 106 + clippy 3-crates + fmt; FE: typecheck/lint/check:i18n 100% (3352 keys ×3)/coverage:js 100% (337 files / 3434 tests)/format). **Still pending: the maintainer's `desktop:build` failed — a working bundled build is the practical gate to real-machine validation + shipping.**

- [x] `WORK-SHELL-GLOBAL-CONCERNS` (2026-07-02) — four user-reported defects that were really ONE architectural smell: cross-cutting window concerns implemented per-screen instead of owned once. On the rebuilt v0.3.0 the user hit: (1) onboarding "found 0 browsers" with no hint that Full Disk Access was the real cause; (2) the window undraggable during onboarding (and every pre-shell screen); (3) every page "one-shot" — leaving the import screen dropped its progress bar though the job kept running, and the bottom bar never showed background work; (4) the onboarding AI step was cosmetic — no choice ever enabled AI. The user's framing ("感覺我們程序像散裝的一樣" / "isn't your unified task view just the Activity page you never finished?") reframed the work: don't patch each screen — own each concern once (window chrome, task progress, discovery status, AI intent). Diagnosed with 4 parallel read-only investigations; each fix run through the mandated loop ([[feedback_subagent_implement_review_loop]]: clean-context Opus implement → INDEPENDENT Opus adversarial review → fix → converge; main agent only orchestrates/gates/commits).
  - **A — surface Full Disk Access denial in onboarding** (backend `90517d8b` + FE `36cd406a`): root cause — `snapshot_browser_profiles` (vault-worker) SWALLOWED any discovery error (incl. macOS TCC `Operation not permitted`) into an empty `Vec`, indistinguishable from "no browsers"; the backup path already classified FDA errors but onboarding did not; and the per-profile classifier only flagged _Safari_ denials as FDA (a stale pre-Sonoma assumption — current macOS TCC-protects all `~/Library/Application Support` browser data too). Backend: a shared `browser_access` classifier + a stable `AppSnapshot.browserDiscoveryIssue` marker (`macos-full-disk-access` / `discovery-error` / null); a keyless O(1) macOS FDA probe (reads a TCC sentinel `~/Library/Safari`) so a denial is caught even when discovery silently returns 0 profiles, with a full truth-table (never nags when FDA is granted or the machine genuinely has no browsers, never fires off-macOS); the per-profile classifier fixed to flag FDA for every family on macOS. FE: an actionable empty-state (grant-FDA callout + System-Settings deep-link + a Re-check button that re-fetches the snapshot), a calm neutral message for a genuine empty machine, an honest error state for `discovery-error`. Reviews caught: a silently-failing/unhandled-rejection re-check (→ in-step error + always re-enable), a Label-in-Name a11y bug, a missing live-region, an unknown-marker disguised as empty.
  - **B — global window drag region** (`8c6b0b76`): the frameless macOS window (`titleBarStyle: Overlay`) had its drag region ONLY inside `AppShell`, so onboarding + lock + the unlock gate + recovery + upgrade + loading couldn't move the window at all. Hoisted ONE drag strip to the app root (`AppBody`), gated on `hasMacOverlayTitlebar()`, rendered above every branch; it carries `data-titlebar-overlay` itself so its 28px height resolves from the single-source token (no drift), sits at z-10000 so the window stays draggable even while the unlock modal (9999) is up, and occupies only the reserved control-free top band (verified per-screen it never covers a button). Redundant shell-local strip removed; topbar header drag kept as an additive main-shell grab area.
  - **C — unified ambient task bar + finish the Activity page** (`4837c33c`): the user's insight — the "unified in-progress tasks" surface IS the Activity page, unfinished, and the bottom bar was backup-only. Root cause was a RENDER-surface gap, not a state gap: the global `archiveTasks` store already held import AND backup (import's data survived navigation), but only backup drove a shell-level render (via `busyOverlay`), while import's only renderer was the page-local wizard gated on `step`/`importing` (reset on unmount). Replaced backup-only `BackgroundProgress` with an `AmbientTaskBar` driven by the task store + runtime jobs, rendered outside the `<Outlet>` so ANY task (import/backup/index/restore/rekey) shows on every route and survives navigation, collapses multiples to a summary, and click-throughs to the Activity page; backup unified onto it (blocking modal actions keep the full-screen overlay); new bottom-slot precedence BusyOverlay → AmbientTaskBar → BackupFailureToast → null. Activity page: a live task now renders LIVE-with-progress instead of the bogus "stale/interrupted" (only an orphaned running ledger row stays stale). Review caught a DATA-SAFETY MEDIUM: a long import could auto-lock the archive mid-write (import didn't set `busyAction`, so idle-lock wasn't suppressed) → fixed by suppressing the idle timer while ANY archive task is active (defer-not-cancel, consistent with backup); + a presence-based aria-live announcer and a defensive fallback so a background overlay can't render backup invisible. Scoped out (backlog): full app-RELOAD recovery of an in-flight import (import progress is push-only; needs a backend "current progress" query) — navigation survival is fully covered.
  - **D — onboarding AI real, off-by-default opt-in** (`f943be8a`): the AI step persisted nothing; whatever the user picked, `config.ai` stayed all-off (the "Set up AI in Settings" button only navigated). Reframed to a genuine Enable/Skip opt-in that respects the standing rule (AI off by default, never nag — nothing pre-selected, Skip is the effortless default; [[feedback_optional_ai_no_nag]]). Enable persists only the static on-device semantic-search tier (`enabled` + `semanticIndexEnabled` + `embeddingProviderId=static-in-app`, via the same config shape Settings uses; assistant/mcp/skill/content-fetch stay off — they need config the user hasn't provided). After the archive initializes + the first backup populates data, a fire-and-forget background setup (new `lib/ipc/semantic-setup.ts`) downloads the on-device model THEN builds the index (mandatory ordering), living in shell-data so it survives leaving onboarding, with the index build surfacing in C's ambient bar; finishing never blocks and always lands on the dashboard. Honest tri-lingual copy explains it's on-device and that the external-model assistant is a separate Settings step. Reviews confirmed the config is camelCase-correct + off-by-default and caught a download-latch hang (a silent progress channel could hang setup forever + strand the process-global download latch → spurious Settings "downloading") — fixed with an idle-watchdog (resets on any event so slow-but-active downloads aren't aborted) that guarantees the latch is always cleared.
  - Gate (authoritative, all green on the combined tree): Rust for A validated at its commit (shared classifier + probe; clippy `-D warnings` clean, vault-core/platform/worker suites pass, the macOS `/dev/shm` migration test the only known-fail); FE typecheck / lint (`--max-warnings 0`) / `format:check` / `check:i18n` 100% (3363 keys × 3) / `coverage:js` global 100% (340 files / 3467 tests). The independent-review loop caught a real defect at EVERY workstream — the coverage≠behavior rule again ([[feedback_coverage_not_behavior]]): all four passed green while the re-check failure, the a11y bug, the auto-lock-mid-import data-safety gap, and the download hang hid. Design/copy honesty per [[feedback_ui_copy_model]]; fluidity honored ([[feedback_frontend_performance]]). **Real-machine acceptance pending** (the maintainer's `desktop:build` still needs fixing to validate on the real `.app`).

## M-UI — UI Craft / shadcn Adoption

- [x] `WORK-UI-SHADCN-ADOPTION` (2026-07-05) — close the shadcn UI-craft gap (`docs/plan/DISCUSSION-ui-craft-shadcn-gap.md`): shadcn was installed but adoption/theming stalled at "foundation only", so most of the UI was hand-rolled — three coexisting button-styling generations, div-based charts, five competing skeleton dialects. User directive "直接做完" the doc's three lines; every change designed first then run through the implement → INDEPENDENT-Opus-review → validate → fix → re-review loop ([[feedback_subagent_implement_review_loop]]; review agents on Opus, visual/copy on Sonnet [[feedback_ui_copy_model]]). Grounded by an 8-reader parallel understanding pass over the REAL current state, which corrected a premise: `tailwind.css`'s `@theme` already aliases most shadcn slots onto paper tokens, so the button work was 5 surgical gaps, not a class rewrite. Branch `feat/ui-craft-shadcn-adoption`.
  - **Line 1 — Button primitive + pilots**: retheme `ui/button.tsx` (`e44a6eee`) to paper tokens with variants covering the six real clusters an app-wide audit found (435 hand-written `<button>`s): filled-primary / neutral-outline (default) / accent / ghost / destructive / link + a 28px icon size + a reduced-motion-safe `loading` prop. Fixed the 5 real gaps (undefined `text-accent-foreground`; bare `border`→currentColor; non-paper shadows; `rounded-xs`; literal `text-white`) and the inert-`danger` bug (destructive uses the `error` tokens — `--color-danger` does not exist). An Opus reviewer caught a dark-mode contrast bug (`text-paper` on `bg-accent` flips near-black → `text-primary-foreground`). Adopted on **Activity /jobs** (`2b2402f4`, all 7 buttons, open-import Link via `asChild`) and **Settings** (`0c31f651`, 26 action buttons across 8 sections, retiring the duplicated `BUTTON_*` consts) — a dedicated sensitive-flow review lens verified passcode/restore/migration handlers + aria carried over byte-for-byte; role=switch/radio controls correctly excluded.
  - **Line 2 — self-built SVG chart primitive**: ADR `docs/design/chart-primitive-tradeoff.md` (self-build, no charting dep — clears the supply-chain gate; revisit visx-not-recharts only for a complex interactive chart). New `src/components/charts/` (geometry helpers + `CalendarHeatmap` + `Sparkline`). Migrated the year-heatmap off its 365 `<div>`s to SVG (`23143c8c`) and RESTORED the a11y that regressed in the paper rewrite: real role=grid>row>gridcell, per-cell accessible names, a WAI-ARIA roving-tabindex keyboard model (reading-order fallback so an isolated day is reachable), WebKit-safe stroke focus; the 5 levels use the semantic accent ramp, not ad-hoc opacity. Deduped the two hand-rolled sparklines onto the shared `Sparkline` + repainted the legacy-var percent bar (`9b160d80`, `buildSparklinePath` deleted). Aggregation stays off the render path (14.4M-safe).
  - **Line 3 — unified Skeleton**: rebuilt `primitives/skeleton.tsx` (`cc6794b9`) onto paper tokens, fixing the recorded 2026-06-14 layout-shift defect — `DashboardSkeleton` + `SkeletonExplorer` now MATCH their real paper layouts (built from the real `PaperCard` primitives), so no jump on load; a 3-round review refined 8 sub-pixel matches. One existing keyframe (`pk-skeleton-pulse`, opacity-only, reduced-motion-neutralized — no 14th keyframe, per the §9 red line); 3 dead exports removed; the orphaned `.pk-skeleton` CSS deleted. The legacy `.skeleton*` family (34 files) + the Intelligence `.intelligence-skeleton` dialect are LEFT intact for the follow-up sweep.
  - **CI cascade fixed in parallel** (pushed straight to `main` via an isolated `main` git worktree, so the shadcn branch stayed clean): the July-5 merge push surfaced `main` red — but `main` had actually been red since 2026-05-28 (5+ weeks). Peeled + fixed layer by layer, each independently reviewed: RustSec audit (`f4476827`, earlier) → 100% Rust coverage on 7 crash-recovery branches in archive migration/restore (`eb5a4659`→`f9f49e3b`; 5 behavioral tests, 0 production change, Docker-Linux-verified) → the unlock gate covering onboarding (`107b9a35`: `showUnlockGate` now requires `initialized` — a real locked archive still gates, onboarding doesn't) + the stale AI-onboarding-step e2e → `test:e2e:desktop-bridge:truth` quarantined (`82c35408`, `test.fixme` both live tests, NO assertion weakened) after an evidenced diagnosis that it's a long-standing webkit2gtk-under-xvfb env-fragility (the `/health` identity is hard-coded ⇒ readiness not identity; passes on macOS; predates the merge) — a user-approved capped-effort quarantine.
  - **Method**: single-phase Workflows (understand → per-line implement/Opus-review-loop/gate), one FE workflow at a time in the main tree with the CI fix + its diagnosis isolated in a `main` worktree — full parallelism, zero tree collisions. The review loop earned its keep repeatedly (dark-mode contrast, `asChild`+loading interactivity, sparkline height/domain-floor regressions, 8 skeleton layout-shifts). Every line: `check:js` / `coverage:js` (100%) / `build` green. Recorded in [[project_ui_shadcn_adoption]].
  - **Deferred → BACKLOG `WORK-UI-SHADCN-SWEEP` + `WORK-CI-DESKTOP-BRIDGE-REENABLE`**: the remaining ~400 hand-written buttons; the Intelligence `.intelligence-skeleton` dialect + 34 legacy `.skeleton` files; a Bar for Top-Concepts; re-enabling `desktop-bridge:truth`; a nondeterministic Vitest coverage-merge flake (`year-review.tsx:773`, an untouched file — re-run passes). **Landing pending**: `main` must be merged into the branch (it lacks the e2e/quarantine CI fixes) then PR'd. CI on `main` after the quarantine (`82c35408`): only `check:mutation` (verified 100 locally) remains behind the quarantined step, so green expected — confirmation via CI run.

## Release — v0.3.0

- [x] `RELEASE-V030` (2026-07-19) — published PathKeep v0.3.0 (user directive; follows the v0.2.0 `workflow_dispatch` + `unsigned_preview=true` path since the repo still has no signing secrets — the tag-push release path fail-fasts by design, same as v0.1.0's red run).
  - **Pre-release CI repairs on `main`**: (1) `b2acdf18` — the demo-image README commit had a double blank line, failing `prettier --check .` at the top of the strict gate (why a docs-only commit turned CI red); (2) `40120594` — new RustSec advisory RUSTSEC-2026-0204 (crossbeam-epoch 0.9.18, invalid pointer deref in `fmt::Pointer`) landed in the advisory DB and would have failed `audit:rust` on every branch — took the patched semver-compatible 0.9.20 (lockfile-only, `f4476827` precedent). Release cut from `40120594` after the Linux strict gate went green on it.
  - **Local rehearsal (macOS)**: everything green — check:js / desktop-contract / fmt / clippy / supply-chain / release:check / coverage:js 100% / build / browser e2e / desktop-bridge truth e2e / desktop-contract JS mutation / `desktop:build:debug` / `release:size-audit` — except 5 known env-only test failures (4× TCC "Operation not permitted" reading real browser dirs from the agent shell, 1× the recorded macOS `/dev/shm` migration test); all pass on the authoritative Linux gate.
  - **Release**: dispatched `Release` (draft, all platforms, unsigned preview) → 4 matrix builds + checksum manifest green → release notes in the v0.2.0 format (highlights: paper redesign; the full optional AI stack; stars; consent-gated enrichment; archive durability; upgrade safety) with real DMG SHA-256s → published; tag `v0.3.0` = `40120594`.
  - **Disk incident during rehearsal**: the desktop-contract Stryker sandbox copied a stale 5.1G `.claude/worktrees/agent-*` checkout (its nested `src-tauri/target` isn't matched by the top-level ignore) and filled the volume to ENOSPC, which also corrupted `node_modules` (restored via `bun install`). Removed the stale worktree (its branch `worktree-agent-abfbb0a070d6dc4ee` still holds the unmerged 6/29 Activity-center experiment) + the crashed sandbox; root-cause fix in this commit: `.claude/**` added to `ignorePatterns` in both Stryker configs.
  - **Post-release**: bumped `main` to `0.4.0-dev` (package.json / Cargo.toml / tauri.conf.json + lockfile; `desktop:build:debug` re-verified green on the prerelease version string).
