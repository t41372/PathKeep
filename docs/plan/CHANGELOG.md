...
- [x] `WORK-ENRICH-1` **BACKEND** (2026-06-21, commit see below; FE pending) — site-content enrichment MVP (GitHub + generic-readable extractors, content-fetch job, FTS5 v3, consent, content_hash wiring). Supersedes `WORK-READABLE-CONTENT-V03-A`.
  ...
  - Carryover → **W-ENRICH-2**: (1) FE — settings consent UI + detail-panel enriched section + search-result enrichment excerpt + i18n×3; (2) server-sent HTTP 429/403 (vs local empty-bucket) currently defers at ~1s ETA → should use a negative-cache cooldown / honor `Retry-After`, separating server-throttle from local-throttle semantics; (3) P2 sources (YouTube/Bilibili metadata, optional LLM summary, X oEmbed).
