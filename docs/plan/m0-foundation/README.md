...
> 2026-04-07 補記：現行 repo-level blocking / release gate 已由 [program/quality-matrix.md](../program/quality-matrix.md) 接管；本頁保留的是 M0 骨架重置的歷史結論，不再單獨定義全域品質政策。
...
- repo 目前的全域品質規則見 [program/quality-matrix.md](../program/quality-matrix.md)；desktop contract slice 仍作為 `bun run check` 內的 targeted sub-gate 存在，但前端 shell 自身的驗收仍需另行維護。
...
- [x] `M0-002` 所有新界面缺口都已補設計稿或已有明確設計決策。（2026-04-07，`WORK-QC-B` prototype / non-prototype state closeout）
