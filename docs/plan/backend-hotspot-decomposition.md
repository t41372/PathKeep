...
| `src-tauri/crates/vault-platform/src/scheduler.rs`                    |                721 | Thin native-scheduler facade after platform-owner split; public worker-facing contract remains here |
| `src-tauri/crates/vault-platform/src/scheduler/macos.rs`              |                597 | macOS launchd owner; below hard limits while owning the highest-risk native schedule behavior       |
| `src-tauri/crates/vault-platform/src/scheduler/windows.rs`            |                474 | Windows Task Scheduler owner; below hard limits                                                     |
...
### Slice 8 — Post-closeout audit and command façade follow-through
...
- `WORK-BE-E` is complete. Remaining backend follow-up candidates are now the lower-priority support owners (`host_artifacts.rs`, `browser-history-parser::chromium`). The former `vault-platform::scheduler` hotspot was closed during the 2026-04-29 scheduled-backup state-machine block: the facade is down to `721` lines, and macOS / Windows / Linux / audit responsibilities now live under `scheduler/` focused owners while preserving command names and serde payloads.
