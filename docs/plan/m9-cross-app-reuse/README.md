...
- [ ] 完整盤點 app 內剩餘的 consumer-local composition 與 duplicated helper / glue code
- [ ] 產出共享 digest / CTA / evidence / focus / review composition 的抽象策略
- [ ] 至少把一輪高價值 shared composition 從多個 consumer 中抽離
- [ ] 把新的 deferred reuse gap 改記 `TODO: M9` / `TODO: M10`，並回寫 source docs / status / backlog
...
- [ ] `M9-001` 掃描 active `src/` / `src-tauri/` surface，列出仍存在 consumer-local composition、duplicated helper 與 near-duplicate read-model glue 的位置
- [ ] `M9-002` 決定哪些 shared composition 應抽成 primitives / helpers / envelopes，哪些保持 route-local
- [ ] `M9-003` 先抽離一輪高價值 shared composition，覆蓋至少兩個以上 consumer
- [ ] `M9-004` 把新的 reuse debt 與下一輪 milestone seed 寫回 `STATUS.md`、`BACKLOG.md`、feature/design docs 與 code TODO
