...
- [ ] **WORK-V03-PAPER-REDESIGN-A** — Paper + Archival Frontend Rebuild (foundation shipped, route sweep pending)
  ...
  - 進度（2026-05-22 update — Browse UX second pass closed out）：
    ...
    - ✅ **Authoritative gate (JS)**：`bun run check:base` + `bun run coverage:js` (99 / 98 statement+branch gate) green after this batch — closeout commits add focused unit tests for `BackgroundProgress`, the new day-insights extras (first/last/peak/longest/topUrls), list-row favicon → og:image → swatch precedence, Browse view-mode persistence, and the `CLOCK_FORMAT_EVENT` reflow path so the JS coverage threshold stays satisfied without exclusions.
    - ⏳ **Rust coverage residual (carryover #21 / #24)**：full `bun run check` still surfaces 100 % Rust line-coverage residuals in `og_images_fetch.rs:215,216,264,265,268,269,292` (BodyReadError::Io + impossible Selector::parse fall-throughs), `archive_flows.rs:214,224,225,226,411,421,428,431,442,444,447,454,477` (refetch worker error paths), and `read_models.rs:135`. These were latent under a pre-existing `dispatch_command_decodes_all_browser_mirror_command_payloads` panic (worker code calling `Runtime::new()` inside the test's current-thread runtime) that aborted the cargo test binary before the coverage walk completed. Today's `9c3b0eb` test-panic fix lets the gate see them honestly. The lines themselves are unchanged from before this batch — closing them belongs to the existing carryover task block, not this Browse UX scope.
