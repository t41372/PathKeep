...
- [ ] **WORK-CI-B** — Core Intelligence Backend Finish Line
  ...
  - 2026-04-17 follow-up：structural stage 的 profile-wide aggregates 現在改成 batch scan：query families 以 `search_events` batches 建構，refind/path-flow/habit 以 `visit_derived_facts` batches 聚合；benchmark artifacts 也新增 corpus stats、peak RSS 與 expired-lease recovery scenario。剩餘 signoff 收斂到 `PG-RD-AI-011` 的 `10M / 14.4M`、visit-derive / daily full-fallback low-RAM chunking、queue recovery RSS、legacy cleanup 與 host integration。
