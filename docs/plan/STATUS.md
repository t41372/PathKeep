...
- [ ] **WORK-AI-3-CHAT-HISTORY** — chat 歷史持久化（`derived/agent.sqlite`）+ history explorer 前端
  ...
  - 驗收：對話可存/載/刪、重啟後留存；`bun run check:base` + coverage 綠；走完 review pipeline。**之後對 LM Studio 跑 chat track 的 live visual QA（streaming + reasoning + tool-use + history）。**
