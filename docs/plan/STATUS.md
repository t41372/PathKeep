...
- [ ] **WORK-BE-B** — Core Intelligence Domain Boundary And Worker Follow-Through
  ...
  - 2026-04-22 AI boundary follow-through：後續 execution slice 也已把 `src-tauri/crates/vault-core/src/ai.rs` 從單個 `2116` 行 giant-file 拆成薄 façade `ai.rs` (`199` 行) 加上 `ai/{control,provider,indexing,ledger,search,read_model}.rs`。provider validation / probe、semantic index build / ledger bookkeeping、semantic search、assistant tool orchestration，現在都已有獨立 owner；worker caller、queue contract、assistant run / semantic search payload shape、以及既有 Rust mutation contract 都維持不變。`WORK-BE-B` 的下一個 focus 因此回到 `intelligence/mod.rs` 的 residual helper clusters 與 worker residual passthrough clustering，而不是再讓 `ai.rs` 充當雜物間。
  - 2026-04-22 worker read-surface follow-through：同一天的下一刀又把 `src-tauri/crates/vault-worker/src/intelligence.rs` 的 residual read-surface passthrough 拆成 `intelligence/{route_queries,section_queries}.rs`。worker parent file 現在只剩 shared helper + re-export façade (`124` 行)，route-first entity reads 與 section-style surface wrappers 都有各自 owner；既有 worker export surface、section id / window metadata、與 frontend caller contract 都維持不變。`WORK-BE-B` 的下一個真正 hotspot 因此回到 `intelligence/mod.rs` 和其餘仍超線的 backend source files，而不是讓 worker façade 再長回去。
