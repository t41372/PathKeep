...
- [ ] **WORK-AI-4C-STATIC-DEDUP** — model2vec static base tier + content-hash 去重（+ enrichment_summary 進 hash）
...
- [ ] **WORK-ENRICH-1** — site enrichment MVP（GitHub + generic extractor + content-fetch job + FTS5 v3 + consent + UI）
...
- [ ] **WORK-AI-5..9（修訂見 doc 05 §9）** — vector store（binary 召回→int8 rescore）+ search + S2 → hybrid + rerank（+starred boost）→ heavy 工作集 tier + GPU opt-in(Metal) + 選擇性/全量重嵌 → agent → code-mode → MCP/skills/consent UX/i18n/hardening（含 W-AI-4b/4a/star 的 carryover）。
