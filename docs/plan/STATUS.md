...
- [ ] **WORK-BE-A** — Backend Hotspot Decomposition And Import Boundary Split
  ...
  - 2026-04-21 progress：第一階段後端審查已完成並落在 `docs/plan/backend-hotspot-decomposition.md`；第一個 execution slice 已把 `src-tauri/crates/vault-core/src/takeout.rs` 拆成 `takeout/{inspect,import_flow,batches,tests}.rs`，保留 import command / batch audit contract 不變。第二個 execution slice 已把 `src-tauri/crates/vault-core/src/archive/mod.rs` 的 canonical ingest boundary 下沉到 `archive/ingest/{mod,parser,writes}.rs`，並讓 `archive::maintenance` 透過明確的 preview helper 使用 parser/watermark contract，而不是偷用內部狀態。第三個 execution slice 把 Takeout import 熱路徑從 inspection preview helper 解耦：`takeout/import_flow.rs` 現在委派 `takeout/payload_import.rs` 直接消費 parser report，不再為 import 額外配置一份 preview rows，source-evidence plan 也改成接手 parser report ownership 而不是 clone 整份 history。第四個 execution slice 又把 batch review/read-model 邊界從 write-side revert/restore 邏輯拆開：`takeout/batch_review.rs` 現在承接 recent-batch read、preview 與 audit-artifact repair，而 `takeout/batches.rs` 已壓回 `412` 行。relevant targeted Rust regressions、`bun run check`、`bun run build` 都已重新跑綠。下一輪 priority 轉到真正的 parser/import streaming contract 與剩餘的 `archive/mod.rs` hotspot 收尾。
