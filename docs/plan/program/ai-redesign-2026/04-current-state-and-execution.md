...
- **`.pkvec` 向量盤 on-disk 格式（W-AI-4a 現況 → 量化決策延後 W-AI-5/S2）**：4a 的 `VectorStore`（`ai/vector_store.rs`）存**原始 little-endian f32**（`8 + 4*dim` bytes/row）。在 14.4M × 1024-dim 尾端 ≈ **59 GB**——太大不能當最終 on-disk rerank tier，也是它**被排除出 export bundle**（rebuildable derived state；見 `migration.rs` `EXPORT_EXCLUSIONS_DOC` 的 `derived/vectors/`）的原因。**保留 f32 是刻意的 4a 選擇**：它是 disk-rerank tier 的無損 SOURCE plane。專案研究建議 on-disk rerank tier 用 **int8 / MRL**；該量化（Turbovec quantization + disk-rerank record 格式）**明確延後到 W-AI-5/S2 決定**，4a **不**做。不可把尾端 59 GB 當穩態接受——它是 rebuild source，設計上在 target 重生而非搬運。
- **Turbovec API（W-AI-5 grounding，2026-06-20 docs.rs）**：主型別 `TurboQuantIndex::new(dim, bits)`（bits=2/4，data-oblivious 免訓練）+ `add(&[f32])`、`search(&self, queries, k) -> SearchResults`（`&self` 可並發）、`write(path)`/`load(path)`、`prepare()`（add 後、查詢前 eager 重建 cache）。**`IdMapIndex` wrapper** 提供穩定 u64 external id + delete-by-id（VectorIndex trait 的 u64 key/remove 走它）。⚠️ **allowlist/bitmask 過濾未出現在 docs.rs 的 search API**（02 §D.1 假設它在 SIMD kernel 內——需對實際 crate source 覆核；若無，VectorIndex 的 `allowlist` 參數改 **post-filter**：search 較大 k' → 濾 allowlist → 取 k）。edition 2021 / MSRV 1.70（我們 1.94 OK）；純 Rust SIMD（add 時須確認無 C build；過 `deny sources` crates.io）。`VectorIndex::{build,append}` → `IdMapIndex` new+add；`remove` → IdMapIndex delete；`save/load` → write/load；`clear` → 重建。
...
- **candle in-app embedding 研究（W-AI-4，2026-06-20）**：candle-transformers 有 **Qwen3** 支援。Qwen3-Embedding-0.6B = Qwen3 decoder 取 last hidden state → **last-token pooling**（用 attention mask 取最後非-pad token）→ **L2-norm**；query 用 instruction prefix（`Instruct: {task}\nQuery: {text}`），document 無 instruction（接 W-AI-0 的 `EmbeddingRole`）。參考實作：HF **text-embeddings-inference** 的 candle backend + Qwen3-Embedding-0.6B model card。棧：`candle-core`/`candle-nn`/`candle-transformers` 0.10.2（MIT/Apache）+ `tokenizers`（HF, Apache）+ `hf-hub`（最新穩定 0.x）。real inference 走 `#[cfg(not(any(test,coverage)))]` + deterministic stub（同 rig pattern）；pooling/normalize pure helper **un-gate 並測**；S1 throughput 走 env-gated 真機 test。candle-transformers transitive 樹大 → 新 advisory 須 triage 進 deny.toml + check-rust-security.mjs 兩處；MKL/Accelerate/CUDA features **一律 OFF**。
- **candle in-app embedding **QUANTIZED** rework（W-AI-4b，2026-06-21）**：F32 路徑（~2.4 GB RAM、6.29 d/s）改為 **GGUF 量化**（owner directive）。`quantized_qwen3::ModelWeights` 的 public `forward` **只回 logits**（套 lm_head、欄位 private）→ 不能取 last-token hidden state；故用 candle-transformers 的 **public 量化積木**（`gguf_file::Content` / `QMatMul::from_weights` / 量化 `RmsNorm::from_qtensor` / `ConcatKvCache` / RoPE / `repeat_kv`）vendored 一個停在 final RMSNorm、回 hidden states 的 Qwen3 decoder（模型可重用、`clear_kv_cache` 每筆重置，免 per-text rebuild）。**bf16 fallback 不需要**。預設量化 = **Q8_0**（官方 `Qwen/Qwen3-Embedding-0.6B-GGUF`；tokenizer/config 來自 base repo），因為它是 parity > 0.99 的最快量化：Q8_0 doc/query parity vs LM Studio = 0.9995/0.9994，**Q4_K_M parity 僅 0.983（< gate）且 candle CPU 上不比 Q8 快**。量化記進 `model_id`（`<repo>:Q8_0`）→ 換量化 = 換 fingerprint = re-embed。**重要測量結果**：candle 0.10.2 CPU 量化 matmul 每次 dequantize→f32，故 **Q8_0 ~1.25 d/s（比 F32 慢 ~5×）但 RAM 砍半（~1.6 GB）**——量化在 candle CPU 上換的是 RAM 不是速度（llama.cpp/LM Studio 有 native int8 kernel，candle 沒有）。→ **model2vec 快層（D3）需求更強、非更弱**：14.4M first-fill 在 candle 上更不可行（target 上數月）。S2 修正：query 模板是 `Instruct: {task}\nQuery:{text}`（**`Query:` 後無空格**，依 model card）；query-role parity 用「LM Studio document-embed 同一條 formatted 字串」當 reference 驗證（LM Studio `/v1/embeddings` 不自動套 instruction，raw vs formatted cosine ~0.73）。S2 dep：tokenizers 由 `onig`→`fancy-regex`（我方不再要求 C backend）+ 對齊 0.22 消除重複版本；但 **candle-core 0.10.2 在自身 manifest hardcode `tokenizers{features=["onig"]}`，feature-unification 仍會啟用 onig**（完全移除需 `[patch]`/fork candle-core，需 ADR，延後）。
