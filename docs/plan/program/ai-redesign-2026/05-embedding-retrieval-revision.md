...
- starred 檢索 boost 權重（過大 → 語義搜尋變書籤列表；需 bounded + 可調，benchmark 驗證）——**W-AI-6**（hybrid + rerank + starred boost）。`VectorIndex` 的 allowlist post-filter 已就位（content_key-keyed），是 starred-facet 的接點。
