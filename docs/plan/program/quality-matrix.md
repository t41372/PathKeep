...
| Chrome desktop bridge smoke      | `bun run test:e2e:desktop-bridge`                                        | 啟動 feature-gated desktop bridge，讓 Chrome / Playwright 驗證真實 Rust command | local / manual / pre-closeout gate；可覆蓋 updater install / relaunch command transport 與 bridge 健康 / disconnect 退化，但不是 mainline blocking path。 |
