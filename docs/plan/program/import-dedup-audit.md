...
- Downloads / search_terms / favicons all supported and now backed by
  end-to-end sidecar scenarios T6-T9.
...
**not** for dedup.
[`e6_url_strings_stored_verbatim_no_normalization`](../../../src-tauri/crates/vault-core/src/archive/ingest/dedup_scenarios_edge_cases/chromium_contracts.rs)
...
- **Sub-millisecond Chrome visit collision (pinned by C_SUB_MS / E5)**: Chrome
  ...
  [`c_sub_ms_same_millisecond_visits_collapsed_by_fingerprint`](../../../src-tauri/crates/vault-core/src/archive/ingest/dedup_scenarios_edge_cases/chromium_contracts.rs)
...
| C_SUB_MS (E5) — Sub-ms fingerprint collision       | [`c_sub_ms_same_millisecond_visits_collapsed_by_fingerprint`](../../../src-tauri/crates/vault-core/src/archive/ingest/dedup_scenarios_edge_cases/chromium_contracts.rs)                                    | Two visits to same URL at same ms but different source_visit_ids — fingerprint partial index collapses to 1 row. Pins known precision limitation.                                                                                                                                |
| E6 — URL canonicalization (no normalization)       | [`e6_url_strings_stored_verbatim_no_normalization`](../../../src-tauri/crates/vault-core/src/archive/ingest/dedup_scenarios_edge_cases/chromium_contracts.rs)                                              | Trailing slash, fragment, mixed case all stored as separate URLs verbatim. Pins contract so future normalization changes are visible.                                                                                                                                            |
| Empty DB × 3 families                              | `empty_{chromium,firefox,safari}_fixture_imports_without_error` in [`empty_and_resilience.rs`](../../../src-tauri/crates/vault-core/src/archive/ingest/dedup_scenarios_edge_cases/empty_and_resilience.rs) | Zero-row fixtures for each family import without error, summary reports 0/0.                                                                                                                                                                                                     |
| R1a — Corrupt random bytes                         | [`r1a_corrupt_random_bytes_returns_error_not_panic`](../../../src-tauri/crates/vault-core/src/archive/ingest/dedup_scenarios_edge_cases/empty_and_resilience.rs)                                           | Random bytes file returns `Err`, not panic — resilience contract.                                                                                                                                                                                                                |
| R1b — Valid SQLite missing tables                  | [`r1b_valid_sqlite_missing_tables_returns_error_not_panic`](../../../src-tauri/crates/vault-core/src/archive/ingest/dedup_scenarios_edge_cases/empty_and_resilience.rs)                                    | Valid SQLite DB without browser tables returns `Err`, not panic — resilience contract.                                                                                                                                                                                           |
| E1 — Epoch timestamp (visit_time_ms = 0)           | [`e1_epoch_timestamp_imports_without_error`](../../../src-tauri/crates/vault-core/src/archive/ingest/dedup_scenarios_edge_cases/time_and_nullable.rs)                                                      | Epoch 0 timestamp stores and round-trips as 0 — pins lower bound of time domain.                                                                                                                                                                                                 |
| E2 — Year-2038 boundary (2^31 seconds)             | [`e2_year_2038_boundary_imports_without_error`](../../../src-tauri/crates/vault-core/src/archive/ingest/dedup_scenarios_edge_cases/time_and_nullable.rs)                                                   | 2038-01-19T03:14:07Z (2,147,483,647,000 ms) round-trips correctly — pins i64 handling above 32-bit overflow.                                                                                                                                                                     |
| E3 — Far-future timestamp (year 9999)              | [`e3_far_future_timestamp_imports_without_error`](../../../src-tauri/crates/vault-core/src/archive/ingest/dedup_scenarios_edge_cases/time_and_nullable.rs)                                                 | Max-range timestamp stores without overflow — pins i64 capacity at the upper extreme.                                                                                                                                                                                            |
| E4 — Negative timestamp (clamped to 0)             | [`e4_negative_timestamp_clamped_to_zero_without_error`](../../../src-tauri/crates/vault-core/src/archive/ingest/dedup_scenarios_edge_cases/time_and_nullable.rs)                                           | All parsers apply `.max(0)` so negative source timestamps import as 0 ms — pins clamping contract.                                                                                                                                                                               |
| E7 — NULL title handling                           | [`e7_null_title_imports_with_null_archive_title`](../../../src-tauri/crates/vault-core/src/archive/ingest/dedup_scenarios_edge_cases/time_and_nullable.rs)                                                 | URL with NULL source title projects as NULL in archive (not empty string) — pins nullable-column contract. Sibling URL with non-NULL title round-trips normally.                                                                                                                 |
| E8 — Unicode byte-identical round-trip             | [`e8_unicode_urls_and_titles_round_trip_byte_identical`](../../../src-tauri/crates/vault-core/src/archive/ingest/dedup_scenarios_edge_cases/unicode_and_flags.rs)                                          | CJK title, percent-encoded path (NOT decoded), and emoji + em-dash all round-trip byte-identical with no NFC/NFD normalization or case folding. Pins international-user contract.                                                                                                |
| E9 — `hidden` URL flag round-trip                  | [`e9_hidden_url_flag_round_trips_for_both_true_and_false`](../../../src-tauri/crates/vault-core/src/archive/ingest/dedup_scenarios_edge_cases/unicode_and_flags.rs)                                        | `hidden = true` source URL (Chrome redirect intermediates) lands as non-zero in archive; `hidden = false` lands as 0. Pins flag-preservation contract that C-series didn't exercise.                                                                                             |
| E10 — Chromium visit_count zero/nonzero            | [`e10_chromium_visit_counts_round_trip_for_zero_and_nonzero_urls`](../../../src-tauri/crates/vault-core/src/archive/ingest/dedup_scenarios_edge_cases/minor_data_integrity.rs)                             | Chrome URL rows with `visit_count = 0` and `visit_count = 7` preserve their lifetime counts and typed counts, even when the zero-count URL has no visit row.                                                                                                                     |
| E11 — Chromium dangling from_visit                 | [`e11_chromium_dangling_from_visit_is_preserved_verbatim`](../../../src-tauri/crates/vault-core/src/archive/ingest/dedup_scenarios_edge_cases/minor_data_integrity.rs)                                     | A `from_visit` value pointing at a missing source visit is stored as-is instead of being rewritten to NULL/0, pinning current referential-evidence behavior.                                                                                                                     |
| E12 — Chromium visit duration value                | [`e12_chromium_visit_duration_value_is_preserved_verbatim`](../../../src-tauri/crates/vault-core/src/archive/ingest/dedup_scenarios_edge_cases/minor_data_integrity.rs)                                    | The source `visits.visit_duration` value passes through to archive `visits.visit_duration_ms` unchanged; this pins current value-preservation semantics.                                                                                                                         |
| E13 — Safari synthesized context evidence          | [`e13_safari_synthesized_context_evidence_persists_boolean_value`](../../../src-tauri/crates/vault-core/src/archive/ingest/dedup_scenarios_edge_cases/minor_data_integrity.rs)                             | Current-schema Safari `history_visits.synthesized = 1` persists to cold `visit_context_evidence` as `context_key = safari.synthesized`, `value_json = true`, with source visit/url ids preserved.                                                                                |
| E14 — Firefox visit_type passthrough               | [`e14_firefox_visit_type_enum_lands_as_transition_type_without_normalization`](../../../src-tauri/crates/vault-core/src/archive/ingest/dedup_scenarios_edge_cases/minor_data_integrity.rs)                 | Firefox `moz_historyvisits.visit_type` values land in archive `visits.transition_type` unchanged, rather than being normalized to Chromium transition values.                                                                                                                    |
| T6 — Chromium downloads sidecar                    | [`t6_chromium_downloads_round_trip_to_archive_downloads_table`](../../src-tauri/crates/vault-core/src/archive/ingest/dedup_scenarios_chromium_sidecars.rs)                                                 | Chromium `downloads` rows produced by the fixture writer land in archive `downloads` with source id, paths, byte counts, state, MIME fields, and Unix-ms start time preserved.                                                                                                   |
| T7 — Chromium keyword search terms sidecar         | [`t7_chromium_keyword_search_terms_land_with_term_text_preserved`](../../src-tauri/crates/vault-core/src/archive/ingest/dedup_scenarios_chromium_sidecars.rs)                                              | `keyword_search_terms` rows land in archive `search_terms` linked to the canonical URL, with `term`, `normalized_term`, `profile_id`, and `keyword_id` preserved.                                                                                                                |
| T8 — Chromium favicons blob dedup                  | [`t8_chromium_favicons_link_to_canonical_url_rows_with_blob_dedup`](../../src-tauri/crates/vault-core/src/archive/ingest/dedup_scenarios_chromium_sidecars.rs)                                             | `Favicons` DB rows map to archive `favicons` page URLs that match canonical URL rows; identical synthetic PNG bytes deduplicate into one `favicon_blobs` row.                                                                                                                    |
| T9 — Chromium icon mapping                         | [`t9_chromium_icon_mapping_resolves_url_to_favicon`](../../src-tauri/crates/vault-core/src/archive/ingest/dedup_scenarios_chromium_sidecars.rs)                                                            | Multiple `icon_mapping` rows for one icon produce separate page-url favicon rows that keep the shared `icon_url`, proving the mapping table, not the icon resource URL, controls page association.                                                                               |
