...
> 2026-04-10 additive amendment: [ADR-007](007-macos-biometric-session-unlock.md) 已在不改寫本決策核心的前提下，讓 macOS 真正接上 Touch ID session unlock。這份 ADR 仍是 App Lock session-only boundary 的 source of truth。
...
| ------- | ------------------------ | -------------------------------------------------------------------------------------- | ---------------------------------------- |
| macOS   | passcode + Touch ID      | Touch ID 現在可解鎖當前 UI session；passcode 仍是 required fallback，不取代 encryption | 以現有 LocalAuthentication 為基礎擴充    |
| Windows | passcode                 | Windows Hello 尚未接到 desktop bridge；UI 必須顯示 truthful unsupported / degradation  | Windows Hello / OS credential prompt     |
| Linux   | passcode                 | 不提供 biometric；維持 passcode-only                                                   | 視 PAM / polkit / desktop stack 能力另議 |

因此目前 shipping 的 biometric boundary 是：

- macOS 真正可用的 Touch ID session unlock
- Windows / Linux 的 truthful capability / degradation state
- 所有平台一致的 passcode fallback
...
- 把真正已落地的 macOS Touch ID 與仍未落地的 Windows / Linux native biometric 明確分開，能保持 trust。
...
- Windows / Linux 的 native biometric 仍未接線；只有 macOS 現在有真正可用的 Touch ID session unlock
