...
| `vault_core::takeout::browser_history`                                   | Browser Direct local DB inspect/import、staging snapshot、import batch / source-evidence / search refresh | `vault-core`             | 2026-04-24 起作為 `/import` Browser Direct backend boundary；不得再把 Safari / Chrome local DB path 送進 Takeout parser。                                                    |
...
| `vault_core::archive::history`                                           | Explorer-visible history recall、lexical/fuzzy/regex pagination、favicon hydration、export rendering      | `vault-core`             | 2026-05-03 maintainability review accepted a staged split; keep public `list_history` / `export_history` / `load_history_favicons` stable and move owners behind the facade. |
