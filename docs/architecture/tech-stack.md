...
| 全文搜尋        | SQLite FTS5 + dependency-free lexical analyzer                 | 核心召回能力，不依賴外部服務；CJK gram / substring 召回不上 embedding    |
...
- **全文召回：`derived/history-search.sqlite` + SQLite FTS5** — 核心功能，不是 AI 附件。M14 使用 dependency-free lowercase / compact lexical normalization、unicode61 prefix FTS、CJK grams 與 trigram FTS；繁簡中文與全形/半形 folding 等待經審核的 OpenCC / Unicode normalization 方案。
...
- 2026-04-29 amendment：v0.1.0 default build 已移除直接 LanceDB / DataFusion / tantivy 依賴，`RUSTSEC-2026-0002` 的 `lru 0.12.x` allowlist 同步刪除；future vector sidecar 重新開放前必須重跑 supply-chain / packaging evidence。
