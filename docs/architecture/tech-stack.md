...
| 全文搜尋        | SQLite FTS5 + pure-Rust OpenCC analyzer                        | 核心召回能力，不依賴外部服務；繁簡/CJK/substring 召回不上 embedding      |
...
- **全文召回：`derived/history-search.sqlite` + SQLite FTS5** — 核心功能，不是 AI 附件。M14 使用 pure-Rust OpenCC normalization、unicode61 prefix FTS 與 trigram FTS。
